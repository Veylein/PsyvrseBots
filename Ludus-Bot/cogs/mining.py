...
class MiningGame:
    ...
    def __init__(self, user_id: int, seed: int | None = None, guild_id: int | None = None, is_shared: bool = False):
        ...
        self.energy = 60
        self.max_energy = 60
        self.last_energy_regen = datetime.now(timezone.utc)
        ...
        self.last_map_regen = datetime.now(timezone.utc)
    ...
    def mine_block(self, x: int, y: int) -> tuple[bool, str] | tuple[bool, str, tuple[int, int]]:
    ...
    def check_map_regeneration(self, bot=None, guild_id: int | None = None) -> bool:
            ...
            if bot:
                ...
                if server_config_cog:
                    ...
                    if check_guild_id:
                        ...
                        if config and not config.get("mining_map_reset", True):
    ...
    def render_map(self, bot=None) -> io.BytesIO:
        ...
        def load_texture(block_type: str) -> Image.Image:
            ...
            try:
                ...
                texture = texture.resize((block_size, block_size), Image.Resampling.LANCZOS)
        ...
        for dy in range(self.height):
            for dx in range(self.width):
                ...
                img.paste(texture, (int(x1), int(y1)), texture if texture.mode == 'RGBA' else None)
                ...
                if (world_x, world_y) in self.ladders:
                    try:
                        ...
                        ladder_img = ladder_img.resize((block_size, block_size), Image.Resampling.LANCZOS)
                ...
                if (world_x, world_y) in self.torches:
                    try:
                        ...
                        torch_img = torch_img.resize((block_size // 2, block_size), Image.Resampling.LANCZOS)
                ...
                for portal_id, portal_data in self.portals.items():
                    if portal_data["x"] == world_x and portal_data["y"] == world_y:
                        try:
                            ...
                            portal_img = portal_img.resize((block_size, block_size), Image.Resampling.LANCZOS)
        ...
        try:
            ...
            player_texture = player_texture.resize((block_size, block_size), Image.Resampling.LANCZOS)
        ...
        if self.energy <= 0:
            ...
            energy_warning_icon = energy_warning_icon.resize((energy_warning_size, energy_warning_size), Image.Resampling.LANCZOS)
            ...
            # Paste energy icon
            icon_x = int(center_x - total_width // 2)
            icon_y = int(center_y - energy_warning_size // 2)
        ...
        if not self.is_shared:
            ...
            if server_config_cog:
                ...
                reset_enabled = config.get("mining_map_reset", True) if config else True
...
class MiningView(discord.ui.LayoutView):
    ...
    async def show_portal_modal(self, interaction: discord.Interaction):
        ...
        async def modal_callback(modal_interaction: discord.Interaction):
            ...
            if self.game.is_shared and len(modal.children) > 1:
                public_input_item = modal.children[1]
                if isinstance(public_input_item, discord.ui.TextInput):
                    public_answer = public_input_item.value.lower()
                else:
                    public_answer = "no"
            ...
            cog = modal_interaction.client.get_cog("Mining") # type: ignore
            if cog:
                cog.save_data()
    ...
    async def inventory_callback(self, interaction: discord.Interaction):
        ...
        if action == "view_inv":
            ...
            if success:
                cog = interaction.client.get_cog("Mining") # type: ignore
                if cog:
                    cog.save_data()
    ...
    async def refresh(self, interaction: discord.Interaction, message: Optional[str] = None,):
                    ...
                    for other_user_id, other_data in world_info["players"].items():
                        if str(other_user_id) != str(self.user_id):
                            ...
                            self.game.other_players[other_user_id] = {
                                "x": other_data["x"],
                                "y": other_data["y"],
                                "last_update": other_data.get("last_update", datetime.now(timezone.utc).isoformat()),
                                "username": username
                            }
...
class OwnerMiningView(discord.ui.LayoutView):
    ...
    async def show_portal_modal(self, interaction: discord.Interaction):
        ...
        async def modal_callback(modal_interaction: discord.Interaction):
            ...
            if hasattr(self, 'refresh'):
                await self.refresh(modal_interaction, f"🌀 Portal '{portal_name}' placed! ({visibility}, {self.game.items['portal']} left)")  # type: ignore
    ...
    async def dev_menu_callback(self, interaction: discord.Interaction):
        ...
        if action == "infinite_energy":
            ...
            self.game.last_map_regen = datetime.now(timezone.utc)
...
class Mining(commands.Cog):
    ...
    async def start_mining(self, ctx, user_id: int, guild_id: int | None = None, dev_mode: bool = False):
