...
class Owner(commands.Cog):
    ...
    async def tune(self, ctx, mode: str):
        ...
        conn = self.get_db()
        c = conn.cursor()
        c.execute("INSERT INTO analytics (created_at, data) VALUES (?, ?)", (datetime.datetime.utcnow().isoformat(), f"tune:{mode}"))
        conn.commit()
        conn.close()
        if hasattr(ctx, 'respond'):
    ...
    async def purge_inactive(self, ctx, days: int = 7):
        ...
        c.execute("SELECT user_id, username FROM users WHERE last_active < ?", (cutoff,))
        inactive = c.fetchall()
        conn.close()
    ...
    async def override(self, ctx, what: str, value: str):
        ...
        if what == "event_title":
            ...
            c.execute("UPDATE events SET title = ? WHERE event_id = ?", (new_title, int(event_id)))
            msg = f"Event {event_id} title set to {new_title}"
        elif what == "rsvp":
            event_id, user_id = value.split(',', 1)
            c.execute("SELECT rsvp_list FROM events WHERE event_id = ?", (int(event_id),))
            row = c.fetchone()
            rsvp_list = row['rsvp_list'].split(',') if row and row['rsvp_list'] else []
            if user_id not in rsvp_list:
                rsvp_list.append(user_id)
                c.execute("UPDATE events SET rsvp_list = ? WHERE event_id = ?", (','.join(rsvp_list), int(event_id)))
            msg = f"User {user_id} added to RSVP for event {event_id}"
        elif what == "momentum":
            user_id, points = value.split(',', 1)
            c.execute("UPDATE users SET momentum = ? WHERE user_id = ?", (int(points), int(user_id)))
            msg = f"User {user_id} momentum set to {points}"
        elif what == "ai_frequency":
            freq = int(value)
            c.execute("INSERT INTO analytics (created_at, data) VALUES (?, ?)", (datetime.datetime.utcnow().isoformat(), f"ai_frequency:{freq}"))
            msg = f"AI frequency set to {freq}"
        elif what == "topic_trigger":
            trigger = value
            c.execute("INSERT INTO analytics (created_at, data) VALUES (?, ?)", (datetime.datetime.utcnow().isoformat(), f"topic_trigger:{trigger}"))
    ...
    async def silence(self, ctx, hours: int = 1):
        ...
        conn = self.get_db()
        c = conn.cursor()
        c.execute("INSERT INTO analytics (created_at, data) VALUES (?, ?)", (datetime.datetime.utcnow().isoformat(), f"silence:{hours}"))
        conn.commit()
        conn.close()
        if hasattr(ctx, 'respond'):
