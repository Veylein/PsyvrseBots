...
class TicketCog(commands.Cog):
    """Simple ticketing system: create private ticket channels, close with transcript."""
    ...
    @app_commands.command(name='ticket-create', description='Create a private ticket channel')
    ...
    @app_commands.command(name='ticket-close', description='Close the current ticket (must be inside ticket channel)')
    ...
    async def ticket_close(self, interaction: discord.Interaction, transcript_channel: Optional[discord.TextChannel] = None):
        ...
        # post transcript if requested
        if transcript_channel:
            try:
                await transcript_channel.send(f'Transcript for {ch.name} (closed by {interaction.user}):')
    ...
    @app_commands.command(name='ticket-settings', description='Configure ticketing for this server')
...
async def setup(bot: commands.Bot):
    ...
    # register commands on the tree (best-effort)
    try:
        bot.tree.add_command(app_commands.Command(name='ticket-create', description='Create a private ticket channel', callback=cog.ticket_create))
    except Exception:
        pass
    try:
        bot.tree.add_command(app_commands.Command(name='ticket-close', description='Close the current ticket', callback=cog.ticket_close))
    except Exception:
        pass
    try:
        bot.tree.add_command(app_commands.Command(name='ticket-settings', description='Configure ticketing', callback=cog.ticket_settings))
    except Exception:
        pass
import discord
from discord.ext import commands
...
class TicketCog(commands.Cog):
    ...
    async def ticket_group(self, interaction: discord.Interaction):
        if interaction.subcommand_passed is None:
            await interaction.response.send_message('Use /ticket create or /ticket close or /ticket settings', ephemeral=True)
    ...
    async def ticket_close(self, interaction: discord.Interaction, transcript_channel: Optional[discord.TextChannel] = None):
        ...
        if transcript_channel:
            try:
                ...
                # send as file if long
                if len(transcript) > 1900:
                    await transcript_channel.send(file=discord.File(fp=discord.utils.sane_split(transcript)[0], filename=f'transcript-{ch.id}.txt'))
                else:
...
async def setup(bot: commands.Bot):
    cog = TicketCog(bot)
    await bot.add_cog(cog)
    # register group explicitly
    try:
        bot.tree.add_command(app_commands.Group(name='ticket', description='Ticketing commands'))
    except Exception:
        pass
