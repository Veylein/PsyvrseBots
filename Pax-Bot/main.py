...
# Optional translator compatibility: prefer `googletrans` if available,
# otherwise fall back to a thin wrapper around `deep_translator.GoogleTranslator`
# or a minimal stub so the translate command doesn't crash at runtime.
try:
    from googletrans import Translator, LANGUAGES  # type: ignore
except Exception:
    # Build a minimal LANGUAGES mapping used by the bot for lookups
    LANGUAGES = {
        "en": "english",
        "es": "spanish",
        "fr": "french",
    ...
    class Translator:
        """Thin compatibility wrapper that uses deep_translator when
        googletrans is not installed. The returned object provides
        `.text` and `.src` attributes like googletrans' result.
        """

        def __init__(self):
            self._impl = GoogleTranslator()

        def translate(self, text, dest="en"):
            translated = self._impl.translate(text, target=dest)
            ...
            # deep_translator doesn't expose detected source language here,
            # so default to 'auto' to indicate unknown.
            return _Res(translated, "auto")
...
POSITIVE_WORDS = [
    "delightful",
    "fabulous",
    "supeletrcalifragilisticexpialidocious",
    "magnificent",
    "spectacular",
    "extraordinary",
    "phenomenal",
...
# Helper accessors for per-guild configured roles/channels
def get_guild_role_obj(guild: discord.Guild, role_key: str):
    """Return a discord.Role for configured role key (e.g. 'positive_role_id').
    Falls back to None when not configured."""
    try:
        role_id = get_config_value(guild.id, f"roles.{role_key}")
        if role_id:
...
def get_guild_channel_obj(guild: discord.Guild, channel_key: str):
    """Return a discord.TextChannel for configured channel key (e.g. 'log_channel_id')."""
    try:
        chan_id = get_config_value(guild.id, f"channels.{channel_key}")
        if chan_id:
...
def initialize_user_data(user_id, guild_id="0"):
    """Return a default user data structure for a new user.

    This helper centralizes the default keys used across the bot so callers
    can safely initialize missing users without duplicating the default shape.
    """
    return {
        "chi": 0,
        "milestones_claimed": [],
...
ARTIFACT_CONFIG = {
    "common": {
        "emojis": ["🪨", "🍃", "💧", "🪵", "⚪"],
        "names": ["Smooth Stone", "Bamboo Lea", "Morning Dew", "Driftwood", "River Pebble"],
        "heal_percent": 10,
        "weight": 100,
    },
    "rare": {
...
BOSS_DATA = {
    "fang": {
        ...
        "damage_min": 20,
        "damage_max": 80,
        "special": {"name": "Bamboo Strike", "description": "ignores defense 10% of the time", "chance": 0.10},
        "rewards": {"key": "Bamboo Key 🌿", "chi": 150, "artifact_chance": 0.0},  # 50% increase from 100
    },
    "ironhide": {
        "name": "Ironhide Titan",
        "emoji": "🪨",
        ...
        "rewards": {"key": "Titan Key ⚙️", "chi": 300, "artifact_chance": 0.0},  # 50% increase from 200
    },
    "emberclaw": {
        ...
        "rewards": {"key": "Eternal Key 🔥", "chi": 750, "artifact_chance": 0.05},  # 50% increase from 500
    },
    "verdant_colossus": {
        ...
        "special": {
            "name": "Nature's Grasp",
            "description": "Roots player in place for 1 turn and reduces healing by 20%",
            "root_chance": 0.15,
        },
        "rewards": {"key": "Verdant Key 🌱", "chi": 1000, "artifact_chance": 0.08},
    },
    "rifter": {
        "name": "Rifter",
        "emoji": "🌀",
        ...
        "rewards": {"chi": 1500, "custom_item": "Rift Shard"},
    },
}
...
PANDA_NPC_TRADERS = {
    "panda_town": {
        ...
        "blacksmith_bo": {
            ...
            "buys": {"Iron Ore": 30, "Silver Ore": 50, "Gold Ore": 100, "Voidstone": 200},
            "sells": {"Iron Sword": 500, "Silver Shield": 800, "Golden Armor": 1500},
    ...
    "lushsoul_cavern": {
        "gem_dealer_jade": {
            ...
            "buys": {"Ruby Quartz": 150, "Sapphire Shard": 200, "Emerald Fragment": 250, "Diamond Dust": 500},
            "sells": {"Gemcutting Kit": 1000, "Crystal Magnifier": 1500},
        ...
        "ore_merchant_rex": {
            ...
            "buys": {"Iron Ore": 35, "Silver Ore": 55, "Gold Ore": 110, "Voidstone": 220, "Titanium Ore": 300},
            "sells": {"Advanced Pickaxe": 2000, "Mining Helmet": 1200, "Ore Detector": 1800},
...
LUSHARMORY_ITEMS = {
    ...
    "A9": {
        ...
        "cost": 60,
        "cost_type": "Lushcore Gem",
        "description": "A bow imbued with spirit energy - deals 90-140 damage",
        "damage_min": 90,
        "damage_max": 140,
        "type": "weapon",
    },
    "A10": {
        "name": "Lushguard Shield",
        "emoji": "🛡️",
        "cost": 70,
        "cost_type": "Echo Quartz",
        "description": "Shield crafted from Lushsoul materials - +20% protection",
        "protection": 20,
        "type": "armor",
    },
}
...
NPC_TRADERS = {
    ...
    "Whiskerbell": {
        ...
        "trades": {
            "Echo Quartz (Bamboo Biscuits)": {"cost": 5, "gives": 3, "gives_item": "Bamboo Biscuits"},
            "Echo Quartz (Zen Tea)": {"cost": 10, "gives": 5, "gives_item": "Zen Tea"},
            "Echo Quartz (Spirit Cake)": {"cost": 15, "gives": 2, "gives_item": "Spirit Cake"},
        },
    },
    "Quartzbeard": {
        "emoji": "🧙",
...
INGREDIENT_CATALOG = {
    ...
    "moonpetal": {
        ...
        "rarity": "common",
        "source": "garden",
        "description": "Glowing petals harvested under moonlight",
    },
    "winterleaf": {
        "emoji": "❄️",
        "rarity": "common",
        "source": "garden",
        "description": "Frost-touched leaves from winter gardens",
    },
    "sunbloom": {
        "emoji": "🌻",
        "rarity": "common",
        "source": "garden",
        "description": "Bright flowers that capture sunlight",
    },
    "moonflower": {
        "emoji": "🌙",
        "rarity": "uncommon",
        "source": "garden",
        "description": "Mystical flower that only blooms at night",
    },
    "chi_blossom": {
        "emoji": "🌸",
        "rarity": "rare",
        "source": "garden",
        "description": "Rare blossom infused with pure chi energy",
    },
    "mistmoss": {
        "emoji": "🌫️",
        "rarity": "uncommon",
        "source": "garden",
        "description": "Mysterious moss that grows in mist",
    },
    "starherb": {
        "emoji": "⭐",
        "rarity": "uncommon",
        "source": "garden",
        "description": "Rare herb that only blooms at night",
    },
    "emberroot": {
        "emoji": "🔥",
        "rarity": "rare",
        "source": "garden",
        "description": "Fiery root that burns but never chars",
    },
    ...
    "crystal_shard": {
        ...
        "rarity": "uncommon",
        "source": "mining",
        "description": "Shimmering crystal from deep caverns",
    },
    ...
    "shadow_stone": {
        ...
        "rarity": "rare",
        "source": "mining",
        "description": "Dark stone infused with shadow energy",
    },
    "phoenix_coal": {
        "emoji": "🔥",
        "rarity": "epic",
        "source": "mining",
        "description": "Coal that never stops burning",
    },
    ...
    "bamboo_essence": {
        ...
        "rarity": "epic",
        "source": "boss:fang",
        "description": "Pure essence from Fang of the Bamboo",
    },
    "titan_core": {
        "emoji": "⚙️",
        "rarity": "epic",
        "source": "boss:ironhide",
        "description": "Hardened core from Ironhide Titan",
    },
    "eternal_flame": {
        "emoji": "🔥",
        "rarity": "mythic",
        "source": "boss:emberclaw",
        "description": "Eternal fire from Emberclaw",
    },
    ...
    "dragon_scale": {
        ...
        "rarity": "legendary",
        "source": "npc_trade",
        "description": "Mystical scale from ancient dragons",
    },
    "void_essence": {
        "emoji": "🌀",
        "rarity": "legendary",
        "source": "npc_trade",
        "description": "Essence from the void itself",
    },
    "liquid_light": {
        "emoji": "✨",
        "rarity": "rare",
        "source": "npc_trade",
        "description": "Bottled light from fallen stars",
    },
}
...
POTION_RECIPES = {
    # === COMMON POTIONS ===
    "healing_potion": {
        "ingredients": {"moonpetal": 3, "sunbloom": 2},
    ...
    # === UNCOMMON POTIONS ===
    "strength_potion": {
        "ingredients": {"emberroot": 2, "iron_ore": 3, "sunbloom": 1},
    ...
    "speed_potion": {
        "ingredients": {"moonflower": 2, "crystal_shard": 2, "winterleaf": 3},
        "chi_cost": 105,  # Reduced by 30%
        "brew_time": 120,
        "success_rate": 0.85,
        "tier": "uncommon",
    ...
    "poison_potion": {
        "ingredients": {"shadow_stone": 1, "mistmoss": 3, "emberroot": 1},
    ...
    # === RARE POTIONS ===
    "resistance_potion": {
        "ingredients": {"titan_core": 1, "iron_ore": 5, "moonflower": 3},  # Boss drop
    ...
    "regeneration_potion": {
        "ingredients": {"bamboo_essence": 1, "chi_blossom": 3, "moonflower": 5},  # Boss drop
        "chi_cost": 350,  # Reduced by 30%
        "brew_time": 300,
        "success_rate": 0.75,
        "tier": "rare",
    ...
    # === EPIC POTIONS ===
    "phoenix_elixir": {
        "ingredients": {"eternal_flame": 1, "phoenix_coal": 2, "liquid_light": 3},  # Mythic boss drop
    ...
    "berserker_fury": {
        "ingredients": {"titan_core": 2, "emberroot": 5, "shadow_stone": 3},
    ...
    # === LEGENDARY POTIONS ===
    "shadow_veil": {
        "ingredients": {"void_essence": 2, "shadow_stone": 5, "chi_blossom": 5},
    ...
    # === MYTHIC POTIONS ===
    "dragon_blessing": {
        "ingredients": {"dragon_scale": 3, "eternal_flame": 2, "void_essence": 2, "liquid_light": 5},
...
GARDEN_SHOP_ITEMS = {
    "bundles": {
        "Sickle Bundle": {
            ...
            "realm": "Pandian Realm",
            "contents": {"Eternal Sickle": 1, "Heartvine": 20, "Fertilizer Bag": 1},
        },
        "Heroic Bundle": {
            "cost": 60000,
            "emoji": "🏆",
            ...
            "realm": "Pandian Realm",
            "contents": {"Heroic Hoe": 1, "Moonshade": 20, "Fertilizer Bag": 1},
        },
        "Scythe Bundle": {
            "cost": 40000,
            "emoji": "🗡️",
            ...
            "realm": "Pandian Realm",
            "contents": {"Silverdew Scythe": 1, "Silverdew": 20, "Fertilizer Bag": 1},
        },
    },
    "tools": {
        "Watering Bucket": {
    ...
    "seeds": {
        ...
        "Sunblossom": {
            ...
            "harvest_chi": 200,
            "grow_time_minutes": 8,
            "description": "A radiant bloom that soaks up sunlight for extra chi.",
            "realm": "Pandian Realm",
        },
        "Twilight Fern": {
            "cost": 7600,
            ...
            "description": "A fern that thrives at dusk, yielding concentrated chi.",
            "realm": "Pandian Realm",
        },
        "Moonshade": {
            "cost": 28000,
...
GARDEN_TOOLS = {
    ...
    "silverdew_scythe": {
        ...
        "flat_chi_bonus": 0,
        "crafting_recipe": {"Silverdew Artifact": 1, "Silver Ore": 5, "Bamboo": 10},
    ...
    "eternal_planters": {
        ...
        "flat_chi_bonus": 0,
        "crafting_recipe": {"Eternal Shard": 1, "Gold Ore": 3, "Heartstone": 2},
    ...
    "moonstone_trowel": {
        ...
        "crafting_recipe": {"Moonstone Artifact": 1, "Platinum Ore": 2, "Dragon Scale": 1},
    ...
    "rift_gardener": {
        ...
        "crafting_recipe": {"Rift Shard": 2, "Cosmic Dust": 5, "Void Crystal": 1},
...
TUTORIAL_QUESTS = [
    {
        "title": "1️⃣ First Steps: Spread Positivity! 🌸",
        "description": "Say 5 positive words in chat to earn chi.",
        "reward": 75,
    },
    {
        "title": "2️⃣ Create Your Garden 🪷",
        "description": "Use P!garden to start your chi farming journey.",
        "reward": 150,
    },
    {
        "title": "3️⃣ Join the Team Spirit 🐼",
        "description": "Create a team with P!team create <name> or join one.",
        "reward": 225,
    },
    {"title": "4️⃣ Challenge a Friend ⚔️", "description": "Start your first duel with P!duel @user.", "reward": 300},
    {
        "title": "5️⃣ Upgrade Your Team Base 🏠",
        "description": "Add a decoration to your team base with P!team base decorate.",
        "reward": 450,
    },
    {
        "title": "6️⃣ Grow Your Garden 🎋",
        "description": "Upgrade your garden tier with P!garden upgrade.",
        "reward": 600,
    },
    {
        "title": "7️⃣ Claim an Artifact ✨",
        "description": "Find and claim an artifact when one spawns with P!find artifact.",
        "reward": 750,
    },
    {
        "title": "8️⃣ Make a Trade 🤝",
        "description": "Trade chi or artifacts with another user using P!trade.",
        "reward": 900,
    },
    {"title": "9️⃣ Defeat a Boss 🐉", "description": "Complete a boss battle with P!boss.", "reward": 1200},
    {"title": "🔟 Tutorial Master 🏆", "description": "Complete all 9 tutorial quests above.", "reward": 1500},
]
...
async def log_event(event_type: str, description: str, guild, color: discord.Color = discord.Color.blue()):
    ...
    try:
        ...
        if log_channel_id:
            log_channel = get_guild_channel_obj(guild_obj, "log_channel_id")

        # Fallback: Use first available text channel with send permissions
        if not log_channel:
            log_channel = next(
                (ch for ch in guild_obj.text_channels if ch.permissions_for(guild_obj.me).send_messages), None
            )

        # Send log embed if we have a valid channel
        if log_channel and isinstance(log_channel, discord.TextChannel):
            embed = discord.Embed(
                title=f"📋 {event_type}", description=description, color=color, timestamp=datetime.now(timezone.utc)
            )
...
active_rift_battle = (
    None  # Format: {"boss_hp": int, "boss_max_hp": int, "participants": {}, "spawned_at": datetime, "channel_id": int}
)
...
def calculate_damage_with_potions(base_damage: int, attacker_id: str, defender_id: str, duel_context: dict) -> tuple:
    ...
    if "shadow_veil" in defender_effects:
        if random.random() < 0.30:
            messages.append("🌑 **DODGED!** Shadow Veil helped evade the attack!")
            return (0, messages)
...
def process_on_attack_effects(attacker_id: str) -> str:
    ...
    if inventory.get("regeneration_potion_active", 0) > 0:
        messages.append("💚 Regeneration active!")
...
def process_poison_effect(victim_id: str) -> str:
    ...
    if inventory.get("poison_potion_active", 0) > 0:
        return "☠️ Poison dealt 5 damage!"
...
def throw_poison_potion(thrower_id: str, target_id: str, duel_context: dict) -> tuple:
    ...
    if random.random() < 0.20:
        ...
        if check_status_immunity(str(target_id), duel_context):
            message = "🐉 Dragon's Blessing blocked the poison!"
            return (False, False, message)

        apply_potion_effect(str(target_id), "poison_potion", duel_context)
        message = "☠️ Poison potion successfully thrown! Target will take 5 HP per attack!"
...
def cleanup_old_logs():
    ...
    cutoff_date = datetime.now(timezone.utc) - timedelta(days=ERROR_LOG_RETENTION_DAYS)
    error_logs = [log for log in error_logs if datetime.fromisoformat(log["timestamp"]) > cutoff_date]
...
def log_error(error_msg, command=None, user=None, message_link=None, error_code=None):
    ...
    global error_log_counter, error_logs
    ...
    if "pets" in chi_data[user_id] and chi_data[user_id]["pets"]:
        for i, pet in enumerate(chi_data[user_id]["pets"]):
            ...
            if "type" not in pet and "id" in pet:
                ...
                old_id_to_type = {"panda": "panda", "tiger": "tiger", "dragon": "dragon"}
...
async def auto_complete_tutorial_quest(user_id, quest_index, channel=None):
    ...
    # Determine reward from structured TUTORIAL_QUESTS data
    if quest_index < 0 or quest_index >= len(TUTORIAL_QUESTS):
        return None

    reward = TUTORIAL_QUESTS[quest_index].get("reward", 0)
    ...
    if channel:
        quest_name = TUTORIAL_QUESTS[quest_index].get("title", f"Quest {quest_index}")
        embed = discord.Embed(
            title="🎉 Tutorial Quest Completed!",
            description=f"**{quest_name}**\n\n"
            f"✨ Reward: **+{reward} chi**\n"
            f"📊 Completed: **{len(completed_quests)}/{len(TUTORIAL_QUESTS)}** tutorial quests",
...
async def check_and_reset_oversized_garden(user_id, user_name="Unknown", guild=None):
    ...
    tier_limits = {"rare": 200, "legendary": 300, "eternal": 500}
...
async def initialize_garden(user_id, tier, user_name="Unknown", guild=None):
    ...
    user_id_str = str(user_id)
    gardens_data["gardens"][user_id_str] = {"tier": tier, "level": 1, "plants": [], "last_watered": {}}
...
def get_pet_bonuses(user_id):
    ...
    bonuses = {"chi_yield": 0.0, "growth_speed": 0.0, "rare_drop": 0.0}
...
def calculate_grow_time(
    base_minutes, has_fertilizer, garden_tier="rare", user_id=None, plant_name=None, garden_data=None
):
...
async def view_achievements(ctx, category: str = ""):
    ...
    for tier in tiers_order:
        ...
        if field_text:
            embed.add_field(
                name=f"{tier_emoji} {tier.capitalize()} Achievements", value="\n".join(field_text), inline=False
            )
    ...
    if not category and categories_data:
        ...
        for cat_name, cat_data in categories_data.items():
            cat_count = len(
                [ach for ach in all_achievements if ach["category"] == cat_name and ach["id"] in user_achievements]
            )
...
async def on_ready():
    ...
    next_event_time = datetime.now(timezone.utc) + timedelta(
        seconds=random.randint(CHI_EVENT_MIN_INTERVAL, CHI_EVENT_MAX_INTERVAL)
    )


# Duplicate/earlier `on_command_error` removed — using later, full handler defined further down
...
async def on_guild_remove(guild):
    ...
    try:
        ...
        await load_data_from_database()
        print("✅ Reloaded data from database after guild removal")
...
async def remind_command(ctx, time_str: str = ""):
    ...
    try:
        ...
        await ctx.send(
            "❌ Invalid time format! Use numbers followed by: `w` (weeks), `d` (days), `h` (hours), `m` (minutes), `s` (seconds)\n"
...
async def help_command(ctx, category: str = None):
    ...
    if category == "chi":
        ...
        embed = discord.Embed(
            title="⚔️ Duel Commands", description="Challenge players to turn-based combat!", color=discord.Color.red()
        )
        embed.add_field(
            name="⚔️ Combat",
            value=(
        ...
        embed.add_field(
            name="💰 Betting", value="`P!duel bet <amount> @user` - Bet chi on a duel (spectators)", inline=False
        )
        embed.add_field(
            name="✨ Artifacts", value="`P!find artifact` - Claim spawned artifacts (heal in duels)", inline=False
        )
        ...
        embed.add_field(
            ...
            value=("`P!shop <rebirth/chi> <add/remove> <item> <price>` - Manage shops"),
            inline=False,
        )
        embed.add_field(
            name="⚔️ Tournament",
...
async def spawn_chi_party(channel):
    ...
    for i in range(CHI_PARTY_TOKEN_COUNT):
        ...
        chi_party_tokens.append({"id": i + 1, "amount": chi_amount, "claimed_by": None})
    ...
    embed.add_field(name="Available Tokens", value=f"{CHI_PARTY_TOKEN_COUNT} tokens remaining", inline=False)
    await channel.send(embed=embed)
...
async def on_message(message):
    ...
    if pax_active and user_id not in blacklisted_users:
        ...
        if (
            len(chi_party_messages) >= CHI_PARTY_THRESHOLD
            and not chi_party_active
            and time_since_last_party >= CHI_PARTY_COOLDOWN
        ):
            channel_counts = {}
...
async def chi_add(ctx, member: discord.Member, amount: int):
    ...
    embed = discord.Embed(
        title="✅ Chi Added", description=f"Added **{amount}** chi to {member.mention}", color=discord.Color.green()
    )
    ...
    try:
        ...
        dm_embed = discord.Embed(
            title="🔔 Chi Add Command Used", color=discord.Color.green(), timestamp=datetime.now(timezone.utc)
        )
        ...
        dm_embed.set_footer(text="Command: P!chiadd")
...
async def chi_remove(ctx, member: discord.Member, amount: int):
    ...
    embed = discord.Embed(
        title="✅ Chi Removed", description=f"Removed **{amount}** chi from {member.mention}", color=discord.Color.red()
    )
    embed.add_field(name="New Chi Balance", value=f"**{new_chi}**")
    await ctx.send(embed=embed)
    if rebirth_msg:
    ...
    try:
        owner = await bot.fetch_user(PSY_DM_ID)
        dm_embed = discord.Embed(
            title="🔔 Chi Remove Command Used", color=discord.Color.red(), timestamp=datetime.utcnow()
        )
        dm_embed.add_field(name="👤 Used By", value=f"{ctx.author} ({ctx.author.id})", inline=False)
        dm_embed.add_field(name="🎯 Target User", value=f"{member} ({member.id})", inline=False)
        ...
        dm_embed.add_field(name="💔 New Balance", value=f"{new_chi} chi", inline=True)
        dm_embed.add_field(name="🏰 Server", value=f"{ctx.guild.name} (ID: {ctx.guild.id})", inline=False)
        dm_embed.set_footer(text="Command: P!chiremove")
        await owner.send(embed=dm_embed)
    except Exception as e:
        print(f"Failed to send DM notification: {e}")
...
async def remove_rebirth(ctx, member: discord.Member, amount: int):
    ...
    if current_rebirths < amount:
        await ctx.send(
            f"❌ Cannot remove {amount} rebirths! {member.mention} only has **{current_rebirths}** rebirths."
        )
        return
    ...
    try:
        ...
        try:
            ...
            if not positive_role:
                ...
                created_roles.append("💚 **Positive Chi** - Earned by using positive words")
            ...
            if not negative_role:
                ...
                created_roles.append("❤️ **Negative Chi** - Given for using negative words")
            ...
            if not quest_role:
                ...
                created_roles.append("🏆 **Quest Completer** - Awarded for completing tutorial quests")
            ...
            await ctx.send(
                "⚠️ I don't have permission to create roles! You can set them up manually later with `P!config`"
            )

        # Save configuration for THIS guild
        update_guild_config(guild.id, channel_updates)
        ...
        if created_channels:
            success_embed.add_field(name="📝 Channels Created", value="\n".join(created_channels), inline=False)
        else:
            success_embed.add_field(name="📝 Channels", value="All channels already exist! ✓", inline=False)

        if created_roles:
            success_embed.add_field(name="🎭 Roles Created", value="\n".join(created_roles), inline=False)
        else:
            success_embed.add_field(name="🎭 Roles", value="All roles already exist! ✓", inline=False)
...
async def config_command(ctx, action: str = None, setting: str = None, *, value: str = None):
    ...
    if not action:
        ...
        # Channels
        channels_text = []
        log_ch = get_guild_channel_obj(ctx.guild, "log_channel_id")
        updates_ch = get_guild_channel_obj(ctx.guild, "updates_channel_id")
        ...
        # Garden channels (stored as a list of IDs)
        garden_chs = []
        for ch_id in config["channels"].get("garden_channels", []):
            try:
                ch = ctx.guild.get_channel(int(ch_id))
                if ch:
                    garden_chs.append(ch)
            except Exception:
                continue
        if garden_chs:
        ...
        # Roles
        roles_text = []
        positive_role = get_guild_role_obj(ctx.guild, "positive_role_id")
        negative_role = get_guild_role_obj(ctx.guild, "negative_role_id")
        quest_role = get_guild_role_obj(ctx.guild, "quest_completer_role_id")
        ...
        embed.add_field(name="🚀 Status", value=f"**Setup:** {setup_status}", inline=False)
        ...
        if setting in ["log_channel", "updates_channel"]:
            ...
            if value.startswith("<#") and value.endswith(">"):
                ...
                try:
                    ...
                    await ctx.send("❌ Invalid channel! Please mention a channel or provide a channel ID.")
                    return
            ...
            if value.startswith("<@&") and value.endswith(">"):
                ...
                try:
                    ...
                    await ctx.send("❌ Invalid role! Please mention a role or provide a role ID.")
                    return
        ...
        help_embed.add_field(
            name="📝 Examples",
            value=("`P!config set log_channel #panda-logs`\n" "`P!config set positive_role @Positive Vibes`"),
            inline=False,
        )
...
async def psy_admin(ctx):
    ...
    embed = discord.Embed(
        title="🔧 PSY Admin Commands", description="Full admin control panel", color=discord.Color.purple()
    )
    embed.add_field(
...
async def psy_give(ctx, member: discord.Member = None, category: str = "", *, item_name: str = ""):
    ...
    if not category or not item_name:
        await ctx.send(
            "❌ Usage: `P!psy give [@user] <category> <name>`\n"
            "**Examples:**\n"
    ...
    if category == "chi":
        try:
            ...
            save_data()
            await ctx.send(
                f"✅ Added **{amount:,} chi** to {target_user.mention}!\n💰 New Balance: **{chi_data[user_id]['chi']:,} chi**"
            )
            return
        except ValueError:
    ...
    if category in ["rebirth", "rebirths"]:
        try:
            ...
            save_data()
            await ctx.send(
                f"✅ Added **{amount} rebirth(s)** to {target_user.mention}!\n🔄 Total: **{chi_data[user_id]['rebirths']}**"
            )
            return
        except ValueError:
    ...
    if category == "pet":
        ...
        save_data()
        await ctx.send(
            f"✅ Added **{PET_DATA[pet_key]['emoji']} {PET_DATA[pet_key]['name']}** to {target_user.mention}'s collection!\n"
    ...
    if category == "weapon":
        ...
        if not weapon_found:
            ...
            chi_data[user_id]["inventory"][formatted_weapon] = (
                chi_data[user_id]["inventory"].get(formatted_weapon, 0) + 1
            )
            save_data()
        ...
        chi_data[user_id]["inventory"][weapon_found["name"]] = (
            chi_data[user_id]["inventory"].get(weapon_found["name"], 0) + 1
        )
        save_data()
    ...
    if category == "potion":
        ...
        if not potion_found:
            await ctx.send(
                "❌ Potion not found! Available potions: **Healing Potion, Phoenix Elixir, Dragon's Blessing**, etc."
            )
            return

        # Add to inventory
        chi_data[user_id]["inventory"][potion_found["name"]] = (
            chi_data[user_id]["inventory"].get(potion_found["name"], 0) + 1
        )
        save_data()
        await ctx.send(
            f"✅ Added **{potion_found['emoji']} {potion_found['name']}**!\n"
    ...
    if category == "artifact":
        ...
        save_data()
        await ctx.send(
            f"✅ Added **{artifact_emoji} {tier.capitalize()} Artifact: {artifact_name}**!\n"
...
async def psy_remove(ctx, category: str = "", *, item_name: str = ""):
    ...
    if not category or not item_name:
        await ctx.send(
            "❌ Usage: `P!psy remove <category> <item>`\n"
            "**Examples:**\n"
...
async def psy_inspect(ctx, member: discord.Member = None):
    ...
    embed.add_field(
        name="📦 Collections", value=f"Inventory Items: **{inv_count}**\nPets: **{pets_count}**", inline=True
    )
...
async def admin_team(ctx, member: discord.Member = None):
    ...
    embed = discord.Embed(
        title=f"🏆 {team_name}", description=f"**{member.display_name}** is a member", color=discord.Color.gold()
    )
...
async def admin_base(ctx, member: discord.Member = None):
    ...
    embed = discord.Embed(title=f"🏠 {team_name}'s Base", color=discord.Color.dark_gold())
...
async def leaderboard(ctx, category: str = "chi"):
    ...
    if category == "chi":
        ...
        embed = discord.Embed(
            title="🐼 Top Chi Earners", description="Top 10 users with the most chi!", color=discord.Color.gold()
        )
        ...
        sorted_rebirths = sorted(chi_data.items(), key=lambda x: x[1].get("rebirths", 0), reverse=True)
        embed = discord.Embed(
        ...
        if "teams" not in teams_data or not teams_data["teams"]:
            ...
            sorted_teams = sorted(teams_data["teams"].items(), key=lambda x: x[1].get("team_score", 0), reverse=True)
            embed = discord.Embed(
                title="👥 Top Teams", description="Top 3 teams with the highest scores!", color=discord.Color.blue()
            )
            for i, (team_id, team_data) in enumerate(sorted_teams[:3], start=1):
                ...
                embed.add_field(name=f"{emoji} #{i} - {team_name}", value=value_text, inline=False)
        ...
        if not sorted_artifacts:
            ...
            for i, (user_id_str, count, artifacts) in enumerate(sorted_artifacts[:7], start=1):
                ...
                if member:
                    ...
                    embed.add_field(name=f"{emoji} #{i} - {member.display_name}", value=value_text, inline=False)
...
async def tutorial_command(ctx):
    ...
    for i, quest in enumerate(TUTORIAL_QUESTS):
        ...
        quest_text = quest.get("title", f"Quest {i}")
        quest_details = quest.get("description", "") + f"\n**Reward:** {quest.get('reward', 0)} chi"

        embed.add_field(name=f"{status} {quest_text}", value=quest_details, inline=False)
...
async def quest_leaderboard(ctx):
    ...
    if not leaderboard_list:
        ...
        for i, (user_id, completed) in enumerate(leaderboard_list[:10], start=1):
            ...
            if member:
                embed.add_field(
                    name=f"{i}. {member.display_name}", value=f"Completed {len(completed)} quests", inline=False
                )
    await ctx.send(embed=embed)
...
async def claim(ctx, *, args: str = ""):
    global chi_party_active, active_food_event, event_active, event_claimer
    user_id = str(ctx.author.id)
    ...
    if args.lower() == "food":
        ...
        await ctx.send(
            f"🍱 {ctx.author.display_name} claimed **{quantity}x {food_name}**! Check your inventory with `P!inv`!"
        )
    ...
    if chi_party_active:
        ...
        if not available_tokens:
            chi_party_active = False
            await ctx.send("❌ All chi party tokens have been claimed!")
            return
...
async def find_artifact(ctx, *, args: str = ""):
    ...
    embed = discord.Embed(
        ...
        "Use artifacts in duels to heal yourself!",
    ...
    if tool_drop_chance < 0.15:
        ...
        if searchable_tools:
            ...
            if found_tool_key:
                ...
                tool_embed.add_field(name="🛠️ Tool Bonuses", value=found_tool["description"], inline=False)
...
async def event_command(ctx, event_type: str = ""):
    ...
    if not event_type:
        await ctx.send(
            "❌ Usage: `P!event <party/chi/food/garden/eternal/legendary/rare/rift/end>`\n"
    ...
    # Small Chi Event
        ...
        embed = discord.Embed(
            ...
            description="A small chi disturbance appeared! Type `P!claim` to interact!\n\n"
    ...
    # Food Event
        ...
        embed = discord.Embed(
            ...
            "Type `P!claim food` to grab it within **1 minute**!\n\n"
            f"💚 Hunger: +{selected_food['hunger_restore']}\n"
            f"❤️ Health: +{selected_food['health_restore']}",
            color=discord.Color.green(),
        )
        embed.set_footer(text="Use food to feed Pax or your pets!")
        await ctx.send(embed=embed)
        ...
        async def expire_food():
            ...
            if (
                active_food_event
                and active_food_event.get("food_name") == food_name
                and not active_food_event.get("claimed_by")
            ):
    ...
    # Garden Event
        ...
        embed = discord.Embed(
            ...
            description="All gardens are blessed with spiritual energy!\n\n"
            "**Duration:** 10 minutes\n"
            "**Effect:** All plants are instantly ready to harvest!\n"
    ...
    # Artifact Events
        ...
        embed = discord.Embed(
            ...
            "Type `P!find artifact` to claim it within **5 minutes**!\n\n"
            f"💊 Healing Power: **{ARTIFACT_CONFIG[event_type]['heal_percent']}% HP** in duels",
            color=(
                discord.Color.purple()
                if event_type == "rare"
                else discord.Color.gold() if event_type == "legendary" else discord.Color.dark_blue()
            ),
        )
    ...
    # Rift Event
        ...
        embed.set_image(
            url="https://tenor.com/view/kung-fu-panda-dragon-power-dragon-energy-dragon-kung-fu-panda-gif-14325240"
        )
    ...
    # End All Events
        ...
        await ctx.send(
            "❌ Invalid event type! Use: `party`, `chi`, `food`, `garden`, `eternal`, `legendary`, `rare`, `rift`, or `end`"
        )
...
async def enter_rift(ctx):
    ...
    chi_data[user_id]["inventory"]["Celestial Warhammer"] = (
        chi_data[user_id]["inventory"].get("Celestial Warhammer", 0) + 1
    )
    ...
    embed = discord.Embed(
        ...
        "**Rewards Received:**\n"
        "⚔️ Celestial Warhammer (Legendary Weapon)\n"
        "🐯 Tiger Battle Pet (3000 HP)\n"
        "💊 Standard Recovery Potion\n\n"
        "💀 **The Rifter** awaits your challenge!\n"
        "Use `P!rift attack <weapon_attack>` to engage!",
        color=discord.Color.purple(),
    )
...
async def rift_attack(ctx, *, attack_name: str = ""):
    ...
    if not attack_name:
        await ctx.send(
            "❌ Specify an attack! Usage: `P!rift attack <weapon_attack>`\n" "Example: `P!rift attack Dragon's Wrath`"
        )
        return
    ...
    if not weapon_attack:
        await ctx.send(
            f"❌ **{attack_name}** is not a valid weapon attack!\n"
            "Use attacks from your legendary weapons (e.g., Dragon's Wrath, Reaper's Harvest)"
        )
        return
    ...
    if active_rift_battle["boss_hp"] <= 0:
        ...
        embed = discord.Embed(
            ...
            "**All warriors who attacked receive:**\n"
            "💎 **1,500 chi**\n"
            "🌀 **Rift Shard** (Sell for 1 rebirth OR upgrade garden to Rift tier!)",
            color=discord.Color.gold(),
        )
        ...
        for participant_id, stats in active_rift_battle["participants"].items():
            if stats["attacks"] > 0:  # Only those who attacked
                ...
                chi_data[participant_id]["inventory"]["Rift Shard"] = (
                    chi_data[participant_id]["inventory"].get("Rift Shard", 0) + 1
                )
        ...
        sorted_participants = sorted(
            active_rift_battle["participants"].items(), key=lambda x: x[1]["damage_dealt"], reverse=True
        )
        top_3 = sorted_participants[:3]

        leaderboard = "\n".join(
            [f"{i+1}. <@{uid}>: {stats['damage_dealt']:,} damage" for i, (uid, stats) in enumerate(top_3)]
        )
        ...
        embed.add_field(
            name="Boss HP",
            value=f"{hp_bar}\n{active_rift_battle['boss_hp']:,}/{active_rift_battle['boss_max_hp']:,} HP",
            inline=False,
        )
        embed.set_footer(
            text=f"Your damage: {active_rift_battle['participants'][user_id]['damage_dealt']:,} | Use P!rift attack <attack> to continue!"
        )
        await ctx.send(embed=embed)
...
async def message_stats(ctx, mode: str = ""):
    ...
    if not mode:
        await ctx.send(
            "❌ Usage: `P!message <total/human/bot/channel>`\n"
        ...
        if mode == "channel":
            ...
            if mode == "total":
                ...
                if channel_breakdown:
                    ...
                    embed.add_field(name="📈 Top 5 Most Active Channels", value=top_channels, inline=False)
...
async def log_command(ctx, mode: str = None, target: str = None):
    ...
    if mode and mode.lower() == "command":
        ...
        filtered_logs = [log for log in error_logs if log.get("command") and command_filter in log["command"].lower()]
        ...
        embed.add_field(name="🔍 Most Common Errors", value=error_breakdown, inline=False)
        ...
        embed.add_field(
            name="📝 Recent Examples", value=examples_text if examples_text else "No examples", inline=False
        )
    ...
    if page_logs:
        ...
        embed.add_field(name="✅ System Health", value="No errors detected in the past 7 days!", inline=False)

    # Navigation footer
    nav_text = (
        f"Page {page}/{total_pages} • Use `P!log {page+1}` for next page"
        if page < total_pages
...
async def artifact_give(ctx, target: discord.Member = None, artifact_type: str = ""):
    ...
    embed = discord.Embed(
        ...
        color=(
            discord.Color.purple()
            if artifact_type == "rare"
            else discord.Color.gold() if artifact_type == "legendary" else discord.Color.dark_blue()
        ),
    )
...
async def reset_all_chi(ctx):
    ...
    try:
        ...
        await log_event(
            ...
            f"{ctx.author.mention} reset EVERYTHING for all {reset_count} users",
            ctx.guild,
            discord.Color.red(),
        )
...
async def spawn(ctx, event_type: str = "positive"):
    ...
    if event_type not in ["positive", "negative"]:
        await ctx.send(
            "❌ Event type must be `positive` or `negative`! Usage: `P!spawn positive` or `P!spawn negative`"
        )
        return
    ...
    if event_active:
        event_active = False
        event_claimer = None
        await current_event_message.edit(
            embed=discord.Embed(
...
async def rbuy(ctx, *, item_name: str = ""):
    ...
    if total_rebirths < found_item["cost"]:
        await ctx.send(
            f"❌ You don't have enough rebirths! You need **{found_item['cost']} rebirth{'s' if found_item['cost'] > 1 else ''}** but only have **{total_rebirths}**."
        )
        return
...
async def cshop(ctx):
    ...
    for idx, item in enumerate(chi_shop_data["items"], start=1):
        embed.add_field(name=f"C{idx}: {item['name']} - {item['cost']} Chi", value="\u200b", inline=False)
...
async def potions(ctx):
    ...
    for potion in potions:
        ...
        embed.add_field(name=f"{emoji} {potion['item_id']}: {potion['name']}", value=f"{description}", inline=False)
...
async def item(ctx, *, item_query: str = ""):
    ...
    if item_query.lower().endswith(" attacks"):
        ...
        if preconfigured_attacks:
            ...
            embed.add_field(
                name="🏛️ Pre-configured Attacks (Pandian Realm)", value="\n".join(attacks_list), inline=False
            )
        ...
        if user_custom_attacks:
            ...
            embed.add_field(name="✨ Your Custom Attacks", value=user_attacks_list, inline=False)

        if not preconfigured_attacks and not user_custom_attacks:
            embed.add_field(
                name="No Custom Attacks",
                value="This weapon has no pre-configured attacks.\n\n"
            ...
            embed.add_field(
                ...
                "⚠️ Can't use the same attack twice in a row!",
                inline=False,
            )
    ...
    if shop_type == "Chi Shop" or shop_type == "Garden Shop":
        ...
        embed.add_field(
            name="💰 Cost", value=f"{found_item['cost']} rebirth{'s' if found_item['cost'] > 1 else ''}", inline=True
        )
...
async def manual_rebirth(ctx):
    ...
    if current_chi < CHI_REBIRTH_THRESHOLD and current_chi > -CHI_REBIRTH_THRESHOLD:
        await ctx.send(
            f"❌ You need at least **{CHI_REBIRTH_THRESHOLD}** chi or **-{CHI_REBIRTH_THRESHOLD}** chi to manually rebirth! Your current chi: **{current_chi}**"
        )
        return
...
async def buy(ctx, *, item_name: str = ""):
    ...
    if current_chi < found_item["cost"]:
        await ctx.send(
            f"❌ You don't have enough chi! You need **{found_item['cost']:,} chi** but only have **{current_chi:,} chi**."
        )
        return
...
async def inventory(ctx):
    ...
    for item_name in purchased_items:
        ...
        if any(w in item_lower for w in ["sword", "katana", "blade", "sta", "dagger", "spear", "bamboo strike"]):
    ...
    for item_name, quantity in inventory_dict.items():
        if quantity > 0:
            ...
            if any(o in item_lower for o in ["ore", "crystal", "voidstone", "gem", "mineral", "stone"]):
                ...
                item_emoji = "🧪"
            elif any(h in item_lower for h in ["herb", "petal", "lea", "blossom", "root", "flower"]):
                item_emoji = "🌿"
    ...
    if food_inventory:
        ...
        embed.add_field(
            name="🍱 Food Inventory", value="\n".join(food_display) if food_display else "No food items", inline=False
        )
    ...
    if pets:
        ...
        for pet in pets:
            pet_display.append(
                f"• 🐾 **{pet.get('name', 'Unknown')}** (HP: {pet.get('health', 0)}/{pet.get('max_health', 100)})"
            )

        embed.add_field(name="🐾 Pets", value="\n".join(pet_display[:5]) if pet_display else "No pets", inline=False)
    ...
    if not isinstance(garden_inventory, dict):
        print(
            f"⚠️ ERROR: garden_inventory is {type(garden_inventory).__name__}, not dict! Resetting for user {user_id}"
        )
        chi_data[user_id]["garden_inventory"] = {}
        save_data()
        garden_inventory = {}
    ...
    if garden_inventory:
        ...
        if garden_display:
            embed.add_field(name="🌸 Garden Items", value="\n".join(garden_display), inline=False)
    ...
    if boss_keys:
        ...
        embed.add_field(name="🔑 Boss Keys", value="\n".join(keys_display) if keys_display else "No keys", inline=False)
    ...
    if potions:
        ...
        embed.add_field(
            name="🧪 Potions", value="\n".join(potion_display[:10]) if potion_display else "No potions", inline=False
        )
    ...
    if ingredients:
        ...
        if ingredient_display:
            embed.add_field(name="🌿 Crafting Ingredients", value="\n".join(ingredient_display[:10]), inline=False)
...
async def tools(ctx):
    ...
    # Determine guild context (use '0' for DMs)
...
async def craft(ctx, *, tool_name: str = ""):
    ...
    for material, required_qty in tool_data["crafting_recipe"].items():
        ...
        if "Ore" in material:
            ...
            "Artifact" in material
            or "Shard" in material
            or "Scale" in material
            or "Crystal" in material
            or "Dust" in material
        ):
    ...
    if missing_materials:
        ...
        embed.add_field(name="Missing:", value="\n".join([f"• {mat}" for mat in missing_materials]), inline=False)
    ...
    embed.add_field(name="✨ Bonuses", value=tool_data["description"], inline=False)
...
async def bundle(ctx, action: str = "", *, bundle_name: str = ""):
    ...
    if action.lower() == "buy" and bundle_name:
        ...
        if user_chi < bundle_cost:
            await ctx.send(
                f"❌ You need **{bundle_cost:,} chi** to buy **{actual_bundle_name}**!\n"
        ...
        embed.add_field(name="📦 Bundle Contents", value="\n".join(items_added), inline=False)
        embed.add_field(name="💰 New Chi Balance", value=f"**{chi_data[user_id]['chi']:,} chi**", inline=False)
...
async def say(ctx, *, text: str = ""):
    ...
    if not text:
        await ctx.send(
            "❌ Please provide text for me to say!\n" "Usage: `P!say <text>`\n" "Example: `P!say Hello everyone!`"
        )
        return
...
async def sell(ctx, seed_name: str = "", quantity: int = 1):
    ...
    if not seed_name:
        await ctx.send(
            "❌ Usage: `P!sell <seed_name> [quantity]`\n"
    ...
    if not matching_seed:
        await ctx.send(
            f"❌ You don't have any **{seed_name}** seeds in your inventory!\n"
            "View your garden inventory with `P!inv` or buy seeds with `P!garden shop`"
        )
        return
    ...
    if current_seeds < quantity:
        ...
        await ctx.send(
            f"❌ You only have **{current_seeds}x {matching_seed}** seeds!\n"
            f"You tried to sell **{quantity}** seeds.\n\n"
            "*nyuom nyuom nyuom thx for da seeds very yum yum in my tum tum* 🐼"
        )
        return
    ...
    await ctx.send(
        f"💰 Sold **{quantity}x {seed_emoji} {matching_seed}** for **{chi_earned:,} chi**!{remaining_text}\n"
        f"New chi balance: **{chi_data[user_id]['chi']:,} chi**"
    )
...
async def chest(ctx, *args):
    ...
    if not args:
        ...
        if not chest_items:
            embed.add_field(name="Empty Chest", value="Use `P!chest <item> add` to store items safely!", inline=False)
        else:
            # Count duplicates
            item_counts = {}
    ...
    if action == "add":
        ...
        if any(keyword in item_lower for keyword in forbidden_keywords):
            await ctx.send(
                f"❌ You cannot store **{item_name}** in the chest! (Chi, rebirths, pets, and locked plants are not allowed)"
            )
            return
        ...
        # Check garden inventory (seeds, tools)
            ...
            if "locked" in str(garden_inventory.get(item_name, "")).lower():
                await ctx.send("❌ Cannot store locked garden plants!")
                return
...
async def quest(ctx, subcommand: str = ""):
    ...
    if subcommand == "list":
        embed = discord.Embed(
            title="📋 Quest List", description="Here are all available quests:", color=discord.Color.blue()
        )
        ...
        for quest in todo_quests:
            ...
            embed.add_field(name=f"❌ {quest['name']}", value=value_text, inline=False)
...
async def feed(ctx, *, item: str = ""):
    ...
    if not (21 <= current_hour or current_hour < 3):
        await ctx.send(
            f"🐼 Pax isn't hungry right now... come back between 9 PM - 3 AM UTC! (It's currently {current_hour}:00 UTC)"
        )
        return

    accepted_items = ["cookie", "bamboo", "honey", "lea", "fish", "rice"]
    item_lower = item.lower().strip()

    if item_lower not in accepted_items:
        await ctx.send(
            f"🐼 Pax sniffs the {item} and walks away... maybe try something else?\n*Hint: Pax likes {', '.join(accepted_items)}*"
        )
        return
...
async def weapons(ctx):
    ...
    for weapon in user_weapons[:5]:
        ...
        if attack_list:
            embed.add_field(name=f"{weapon['emoji']} {weapon['name'][:30]}", value="\n".join(attack_list), inline=False)
...
async def duel_tutorial(ctx):
    ...
    page2 = discord.Embed(
        title="⚔️ DUEL TUTORIAL - Page 2/5", description="**How to Challenge Someone**", color=discord.Color.orange()
    )
    page2.add_field(name="1️⃣ Challenge", value="`P!duel @username`\nChallenges a user to duel", inline=False)
    page2.add_field(
    ...
    # Page 3: Combat
    page3 = discord.Embed(
        title="⚔️ DUEL TUTORIAL - Page 3/5", description="**Fighting in a Duel**", color=discord.Color.gold()
    )
    page3.add_field(
    ...
    page3.add_field(
        name="🐼 Pet Attacks", value="`P!pet attack`\nUse your pet's special move! (if you have a pet)", inline=False
    )
    ...
    # Page 4: Advanced
    page4 = discord.Embed(
        title="⚔️ DUEL TUTORIAL - Page 4/5", description="**Advanced Tactics**", color=discord.Color.purple()
    )
    page4.add_field(
    ...
    # Page 5: Spectating
    page5 = discord.Embed(
        title="⚔️ DUEL TUTORIAL - Page 5/5", description="**Spectating & Betting**", color=discord.Color.blue()
    )
    page5.add_field(
        name="💰 How to Bet",
...
async def duel(ctx, action_or_user: str = "", *, item_name: str = ""):
    ...
    if action_or_user == "bet":
        ...
        active_duel["bets"].append(
            {"bettor_id": ctx.author.id, "bet_amount": bet_amount, "bet_on_user_id": bet_on_user.id}
        )
    ...
    if ctx.message.mentions and action_or_user not in ["bet", "accept", "deny", "attack", "heal", "run"]:
        ...
        if challenger_id in teams_data["user_teams"] and target_id in teams_data["user_teams"]:
            ...
            if target_team_id in challenger_team.get("allies", []):
                ...
                await ctx.send(
                    f"🤝 You cannot duel members of **{target_team_name}** - your teams are allies!\n"
                    "💡 Allied teams can train together but not duel each other."
                )
                return
    ...
    if action_or_user == "deny":
        ...
        await ctx.send(
            f"🐼 *Aw sorry... {ctx.author.mention} doesn't want to battle you, {challenger.mention}. How about we try peace, if that doesn't work then we can always use fists!*"
        )
        return
    ...
    if action_or_user == "attack":
        ...
        if not item_name:
            await ctx.send(
                "❌ Usage: `P!duel attack <weapon_id>` or `P!duel attack <weapon> <attack>`\nExample: `P!duel attack K1` or `P!duel attack Katana Swift Slash`"
            )
            return
        ...
        if weapon_id_result:
            ...
            if not custom_attack:
                await ctx.send(
                    f"❌ You must specify an attack! Use `P!duel attack <weapon_id>` (e.g., K1) or `P!duel attack {item_found['name']} <attack_name>`"
                )
                return
        ...
        if custom_attack:
            ...
            if last_attack and last_attack.lower() == custom_attack.lower():
                await ctx.send(
                    f"❌ You can't use the same attack **{custom_attack}** twice in a row! Choose a different attack."
                )
                return
            ...
            if attack_found:
                ...
                await ctx.send(
                    f"⚔️ {ctx.author.mention} used **{weapon_name}** to unleash *{custom_attack}* and dealt **{modified_damage} damage** to {opponent.mention}! 🏛️"
                )
            else:
                ...
                await ctx.send(
                    f"⚔️ {ctx.author.mention} used **{weapon_name}** to unleash *{custom_attack}* and dealt **{modified_damage} damage** to {opponent.mention}!"
                )
        else:
            await ctx.send(
                f"⚔️ {ctx.author.mention} attacked with **{item_found['name']}** and dealt **{damage} damage** to {opponent.mention}!"
            )
        ...
        if opponent_hp <= 0:
            ...
            if winner_id in teams_data["user_teams"]:
                ...
                teams_data["teams"][winner_team_id]["team_score"] = (
                    teams_data["teams"][winner_team_id].get("team_score", 0) + 1
                )
                save_teams()
            ...
            embed = discord.Embed(title="🏆 DUEL COMPLETE!", description=description, color=discord.Color.gold())
            await ctx.send(embed=embed)
        else:
    ...
    if action_or_user == "run":
        ...
        embed = discord.Embed(title="🏃 COWARD!", description=description, color=discord.Color.dark_grey())
        await ctx.send(embed=embed)
        return
...
async def boss(ctx, action: str = "", *, item_name: str = ""):
    ...
    if action == "list":
        ...
        for boss_key, boss_info in BOSS_DATA.items():
            ...
            artifact_text = (
                f" + {int(rewards['artifact_chance']*100)}% unique artifact" if rewards["artifact_chance"] > 0 else ""
            )
    ...
    if action == "fight":
        ...
        if boss_key not in BOSS_DATA:
            await ctx.send(
                f"❌ Unknown boss! Use `P!boss list` to see all available bosses.\nValid names: {', '.join(BOSS_DATA.keys())}"
            )
            return
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                "*Use `P!boss attack <weapon>` to strike!*"
            ),
            color=discord.Color.red(),
        )
    ...
    if action == "attack":
        ...
        if not item_name:
            await ctx.send(
                "❌ Usage: `P!boss attack <weapon>` or `P!boss attack <weapon_id>`\nExample: `P!boss attack Katana` or `P!boss attack K1`"
            )
            return
        ...
        if active_boss_battle["boss_hp"] <= 0:
            ...
            if boss_key == "fang":
                chi_data[user_id]["inventory"]["bamboo_essence"] = (
                    chi_data[user_id]["inventory"].get("bamboo_essence", 0) + essence_amount
                )
                ingredient_drops.append(f"🎋 Bamboo Essence x{essence_amount}")
            elif boss_key == "ironhide":
                chi_data[user_id]["inventory"]["titan_core"] = (
                    chi_data[user_id]["inventory"].get("titan_core", 0) + essence_amount
                )
                ingredient_drops.append(f"⚙️ Titan Core x{essence_amount}")
            elif boss_key == "emberclaw":
                chi_data[user_id]["inventory"]["eternal_flame"] = (
                    chi_data[user_id]["inventory"].get("eternal_flame", 0) + essence_amount
                )
            ...
            if random.random() < 0.50:
                ...
                chi_data[user_id]["inventory"]["phoenix_coal"] = (
                    chi_data[user_id]["inventory"].get("phoenix_coal", 0) + bonus_count
                )
            ...
            if random.random() < 0.70:
                ...
                chi_data[user_id]["inventory"][common_ing] = (
                    chi_data[user_id]["inventory"].get(common_ing, 0) + common_count
                )
                ing_data = INGREDIENT_CATALOG.get(common_ing, {})
                ingredient_drops.append(
                    f"{ing_data.get('emoji', '💎')} {common_ing.replace('_', ' ').title()} x{common_count}"
                )
            ...
            await log_event(
                "Boss Victory",
                f"**Champion:** {ctx.author.mention} ({ctx.author.name})\n**Boss:** {boss_info['name']}\n**Rewards:** {rewards['key']} + {rewards['chi']} chi{' + ' + artifact_won if artifact_won else ''}",
                ctx.guild,
                discord.Color.gold(),
            )
            ...
            embed = discord.Embed(
                title=f"{boss_info['emoji']} BOSS DEFEATED!", description=description, color=discord.Color.gold()
            )
        ...
        if (
            boss_info["special"].get("trigger_turn")
            and active_boss_battle["turn"] % boss_info["special"]["trigger_turn"] == 0
        ):
        ...
        if (
            "burn_damage" in boss_info["special"]
            and active_boss_battle["burn_turns"] == 0
            and active_boss_battle["turn"] == 1
        ):
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                "*Your turn! Use `P!boss attack <weapon>` or `P!boss heal <item>`*"
            ),
            color=discord.Color.orange(),
        )
        await ctx.send(embed=embed)
    ...
    if action == "heal":
        ...
        if not item_found:
            ...
            for artifact in user_artifacts:
                if artifact["name"].lower() == item_name.lower():
                    ...
                    active_boss_battle["player_hp"] = min(
                        active_boss_battle["player_hp"] + base_healing, active_boss_battle["player_max_hp"]
                    )
                    ...
                    if (
                        ...
                        heal_amount = int(boss_info["hp"] * boss_info["special"]["heal_percent"])
                        active_boss_battle["boss_hp"] = min(
                            active_boss_battle["boss_hp"] + heal_amount, boss_info["hp"]
                        )
                        heal_message += f"\n🛡️ **{boss_info['special']['name']}!** Boss healed {heal_amount} HP!"
            ...
            await ctx.send(
                f"❌ **{item_name}** is not a valid healing item! Use healing items from Chi Shop or artifacts."
            )
            return
        ...
        old_hp = active_boss_battle["player_hp"]
        active_boss_battle["player_hp"] = min(
            active_boss_battle["player_hp"] + healing, active_boss_battle["player_max_hp"]
        )
        actual_healing = active_boss_battle["player_hp"] - old_hp
...
async def train(ctx, action_or_user: str = "", mode_or_item: str = "", *, item_name: str = ""):
    ...
    if ctx.message.mentions:
        ...
        if challenger_team_id == target_team_id:
            ...
            # Enemy teams - cannot train
            await ctx.send(
                f"⚔️ You cannot train with members of **{target_team['name']}** - your teams are enemies!\n"
                "💡 Enemy teams can duel each other but not train together."
            )
            return
        else:
            # Neutral teams - cannot train
            await ctx.send(
                "🥊 You can only train with teammates or allied teams!\n"
                f"**{challenger_team['name']}** and **{target_team['name']}** are not allies.\n"
                "💡 Use `P!team ally add @team_leader` to form an alliance!"
            )
            return
    ...
    if action_or_user == "deny":
        ...
        await ctx.send(
            f"🐼 *Aw sorry... {ctx.author.mention} doesn't want to train with you, {challenger.mention}. Maybe another time!*"
        )
        return
    ...
    if action_or_user == "attack":
        ...
        if active_npc_training is not None:
            ...
            if not actual_item_name:
                await ctx.send(
                    "❌ Usage: `P!train attack <weapon_id>` or `P!train attack <weapon> <attack>`\nExample: `P!train attack K1` or `P!train attack Katana Swift Slash`"
                )
                return
            ...
            if weapon_id_result:
                ...
                if not custom_attack:
                    await ctx.send(
                        f"❌ You must specify an attack! Use `P!train attack <weapon_id>` (e.g., K1) or `P!train attack {item_found['name']} <attack_name>`"
                    )
                    return
            ...
            if custom_attack:
                await ctx.send(
                    f"🎯 {ctx.author.mention} used **{item_found['name']}** to unleash *{custom_attack}* and dealt **{damage} damage** to {npc_info['emoji']} {npc_info['name']}!"
                )
            else:
                await ctx.send(
                    f"🎯 {ctx.author.mention} attacked with **{item_found['name']}** and dealt **{damage} damage** to {npc_info['emoji']} {npc_info['name']}!"
                )

            # Check if NPC is defeated
            if active_npc_training["npc_hp"] <= 0:
                chi_reward = {"dummy": 0, "easy": 40, "medium": 75, "hard": 150}[
                    active_npc_training["npc_key"]
                ]  # Moderate 50-60% increase
                if chi_reward > 0:
                ...
                embed = discord.Embed(
                    ...
                    description=(
                        f"{ctx.author.mention} defeated **{npc_info['emoji']} {npc_info['name']}**!\n\n**Reward:** +{chi_reward} chi"
                        if chi_reward > 0
                        else f"{ctx.author.mention} defeated the Training Dummy!\n\n*Great practice session!*"
                    ),
                    color=discord.Color.green(),
                )
                await ctx.send(embed=embed)
        ...
        if custom_attack:
            ...
            if attack_found:
                ...
                await ctx.send(
                    f"🥊 {ctx.author.mention} used **{weapon_name}** to unleash *{custom_attack}* and dealt **{modified_damage} damage** to {opponent.mention}!"
                )
            else:
                # User-created custom attack - use hash-based variation
                # Add custom attack if not already in list
            ...
            await ctx.send(
                f"🥊 {ctx.author.mention} attacked with **{item_found['name']}** and dealt **{damage} damage** to {opponent.mention}!"
            )
        ...
        if opponent_hp <= 0:
            ...
            embed = discord.Embed(title="🏆 TRAINING COMPLETE!", description=description, color=discord.Color.blue())
            await ctx.send(embed=embed)
        else:
    ...
    if action_or_user == "heal":
        ...
        if active_npc_training is not None:
            ...
            if not item_found:
                await ctx.send(
                    f"❌ '{actual_item_name}' is not a valid healing item! Check your inventory with `P!inv`"
                )
                return
            ...
            active_npc_training["player_hp"] = min(
                active_npc_training["player_hp"] + healing, active_npc_training["player_max_hp"]
            )
            actual_healing = active_npc_training["player_hp"] - old_hp

            npc_info = NPC_DATA[active_npc_training["npc_key"]]
            await ctx.send(
                f"💚 {ctx.author.mention} used **{item_found['name']}** and healed for **{actual_healing} HP**!"
            )
        ...
        if ctx.author.id == active_training["challenger"]:
            ...
            active_training["challenger_hp"] = min(
                active_training["challenger_hp"] + healing, active_training["challenger_max_hp"]
            )
            actual_healing = active_training["challenger_hp"] - old_hp
            active_training["turn"] = active_training["challenged"]
        else:
            old_hp = active_training["challenged_hp"]
            active_training["challenged_hp"] = min(
                active_training["challenged_hp"] + healing, active_training["challenged_max_hp"]
            )
...
async def npc(ctx, action: str = "", npc_name: str = "", item_name: str = "", quantity: int = 1):
    ...
    if action.lower() not in ["sell", "buy"]:
        ...
        embed = discord.Embed(
            ...
            description="*\"{found_npc['dialogue']}\"*\n\n📍 Location: {found_npc['location']}",
            color=discord.Color.green(),
        )
    ...
    if action.lower() == "sell":
        if not npc_name or not item_name:
            await ctx.send(
                "❌ Usage: `P!npc sell <npc_name> <item> [quantity]`\n" "Example: `P!npc sell Mei Lavender 5`"
            )
            return
        ...
        if item_name not in found_npc["buys"]:
            await ctx.send(
                f"❌ {found_npc['name']} doesn't buy **{item_name}**!\n"
        ...
        await ctx.send(
            f"✅ Sold **{quantity}x {item_name}** to {found_npc['emoji']} **{found_npc['name']}**!\n"
            f"💰 Earned: **{total_chi:,} chi**\n"
    ...
    if action.lower() == "buy":
        if not npc_name or not item_name:
            await ctx.send(
                "❌ Usage: `P!npc buy <npc_name> <item> [quantity]`\n" 'Example: `P!npc buy Mei "Healing Potion" 2`'
            )
            return
        ...
        if item_name not in found_npc["sells"]:
            await ctx.send(
                f"❌ {found_npc['name']} doesn't sell **{item_name}**!\n"
        ...
        if chi_data[user_id]["chi"] < total_cost:
            await ctx.send(
                f"❌ You need **{total_cost:,} chi** to buy **{quantity}x {item_name}**!\n"
        ...
        await log_event(
            ...
            f"**Cost:** {total_cost:,} chi",
            ctx.guild,
            discord.Color.green(),
        )

        await ctx.send(
            f"✅ Bought **{quantity}x {item_name}** from {found_npc['emoji']} **{found_npc['name']}**!\n"
...
async def lusharmory(ctx, *, item_query: str = ""):
    ...
    if found_item["cost_type"] == "chi":
        ...
        if current_chi < found_item["cost"]:
            await ctx.send(
                f"❌ You don't have enough chi! You need **{found_item['cost']} chi** but only have **{current_chi}**."
            )
            return

        chi_data[user_id]["chi"] -= found_item["cost"]
        chi_data[user_id]["inventory"][found_item["name"]] = (
            chi_data[user_id]["inventory"].get(found_item["name"], 0) + 1
        )
        save_data()
        ...
        await log_event(
            ...
            f"{ctx.author.mention} purchased **{found_item['name']}** for {found_item['cost']} chi",
            ctx.guild,
        ...
        if ore_count < found_item["cost"]:
            await ctx.send(
                f"❌ You don't have enough {required_ore}! You need **{found_item['cost']}** but only have **{ore_count}**."
            )
            return
        ...
        embed.add_field(
            name=f"Remaining {required_ore}",
            value=str(chi_data[user_id]["inventory"].get(required_ore, 0)),
            inline=True,
        )
        await ctx.send(embed=embed)
        ...
        await log_event(
            "🛡️ Lusharmory Forge",
            f"{ctx.author.mention} forged **{found_item['name']}** using {found_item['cost']} {required_ore}",
            ctx.guild,
            discord.Color.dark_purple(),
        )
...
async def mine(ctx, target: str = ""):
    ...
    if target.lower() == "ore":
        if not has_pickaxe:
            await ctx.send(
                "❌ You need a **Pickaxe** to mine for ores! Purchase one at the Lusharmory (`P!lusharmory`)."
            )
            return
        ...
        await log_event(
            "⛏️ Ore Mining",
            f"{ctx.author.mention} mined for ores in Lushsoul Cavern",
            ctx.guild,
        ...
        await log_event(
            "⛏️ Cavern Mining", f"{ctx.author.mention} mined in Lushsoul Cavern", ctx.guild, discord.Color.purple()
        )
...
async def level(ctx, user: discord.Member = None):
    ...
    embed.add_field(name="💪 Level Bonus", value=f"+{user_level * 5}% damage and healing", inline=False)
            ...
            if guild:
                ...
                if channel and isinstance(channel, discord.TextChannel):
                    ...
                    if challenger and challenged:
                        ...
                        embed = discord.Embed(
                            title="⏱️ DUEL TIMEOUT!", description=description, color=discord.Color.dark_grey()
                        )
                        await channel.send(embed=embed)
            ...
            if guild:
                ...
                if channel and isinstance(channel, discord.TextChannel):
                    ...
                    if challenger and challenged:
                        ...
                        embed = discord.Embed(
                            title="⏱️ TRAINING TIMEOUT!", description=description, color=discord.Color.dark_grey()
                        )
                        await channel.send(embed=embed)
...
async def artifact_spawner():
    ...
    for guild in bot.guilds:
        # Prefer configured log channel, fallback to first available sendable text channel
        channel = get_guild_channel_obj(guild, "log_channel_id")
        if not channel:
            channel = next((ch for ch in guild.text_channels if ch.permissions_for(guild.me).send_messages), None)
...
async def daily_chi_evaluation():
    ...
    if now.hour == 23 and now.minute == 59:
        if not bot.guilds:
            return
        guild = bot.guilds[0]
        channel = get_guild_channel_obj(guild, "updates_channel_id")
        if not channel:
            return

        positive_role = get_guild_role_obj(guild, "positive_role_id")
        ...
        if quest in reaction.message.content:
            if i not in quest_data["user_progress"][user_id]:
                ...
                if len(quest_data["user_progress"][user_id]) == len(quest_data["quests"]):
                    quest_role = get_guild_role_obj(reaction.message.guild, "quest_completer_role_id")
                    if quest_role:
    ...
    if active_team_duel and isinstance(action_or_target, str):
        ...
        if action == "accept":
            ...
            embed = discord.Embed(
                ...
                "Leaders command your teams to victory!",
                color=discord.Color.red(),
            )
            embed.add_field(name=f"🔵 {challenger_team['name']}", value=challenger_roster, inline=True)
            embed.add_field(name=f"🔴 {challenged_team['name']}", value=challenged_roster, inline=True)
            embed.add_field(
                name="⚡ Turn",
            ...
            await ctx.send(
                f"❌ **{challenged_team['name']}** declined the challenge from **{challenger_team['name']}**!"
            )
            active_team_duel = None
            return
...
async def tduel_action(ctx, action: str = "", member_num: str = "", *, target_or_item: str = ""):
    ...
    current_leader = (
        active_team_duel["challenger_leader"] if is_challenger_turn else active_team_duel["challenged_leader"]
    )
    ...
    my_members = (
        active_team_duel["challenger_members"] if is_challenger_turn else active_team_duel["challenged_members"]
    )
    enemy_members = (
        active_team_duel["challenged_members"] if is_challenger_turn else active_team_duel["challenger_members"]
    )
    ...
    if action == "status":
        ...
        embed.add_field(name=f"🔵 {challenger_team['name']}", value=challenger_roster or "No members", inline=True)
        embed.add_field(name=f"🔴 {challenged_team['name']}", value=challenged_roster or "No members", inline=True)
        embed.add_field(name="⚡ Current Turn", value=f"{current_leader_user.mention}'s turn to command!", inline=False)
        ...
        if not member_num or not target_or_item:
            await ctx.send(
                "❌ Usage: `P!tduel attack <your_member#> <enemy#> <weapon>`\n"
        ...
        next_leader = await bot.fetch_user(
            active_team_duel["challenged_leader"] if is_challenger_turn else active_team_duel["challenger_leader"]
        )
        ...
        target = await bot.fetch_user(int(target_id))
        action_text = (
            f"💚 {target.display_name} was healed with **{healing_item['name']}** for **{actual_healing} HP**!"
        )
...
TEAM_BOSS_DATA = {
    "guardian": {
        ...
        "damage_min": 40,
        "damage_max": 120,
        "special": {"name": "Root Entangle", "description": "Stuns a random team member for 1 turn", "chance": 0.15},
        "rewards_per_member": {"chi": 1000, "artifact_chance": 0.25},
    ...
    "titan": {
        ...
        "rewards_per_member": {"chi": 2000, "artifact_chance": 0.35},
    ...
    "dragon": {
        ...
        "rewards_per_member": {"chi": 3500, "artifact_chance": 0.50},
...
async def teamboss(ctx, boss_name: str = ""):
    ...
    if boss_name not in TEAM_BOSS_DATA:
        await ctx.send("❌ Invalid boss! Choose from: guardian, titan, dragon\nUse `P!teamboss` to see all bosses.")
        return
    ...
    embed = discord.Embed(
        ...
        "The boss scales with your team size - fight strategically!",
        color=discord.Color.dark_purple(),
    )
    embed.add_field(name=f"👥 {team['name']} ({team_size} members)", value=team_roster, inline=True)
    embed.add_field(
...
async def tboss_action(ctx, action: str = "", member_num: str = "", *, target_or_item: str = ""):
    ...
    if action == "status":
        ...
        for i, member_id in enumerate(team["members"], 1):
            ...
            status_icon = (
                "💀"
                if hp_data["status"] == "ko"
                else "🛡️" if hp_data["status"] == "defending" else "🔥" if hp_data["status"] == "burning" else "⚡"
            )
        ...
        embed.add_field(name=f"👥 {team['name']}", value=team_roster or "Team wiped!", inline=True)
        embed.add_field(
            name=f"{boss_template['emoji']} {boss_template['name']}",
        ...
        if active_team_boss_battle["boss_hp"] <= 0:
            ...
            if artifacts_earned:
                embed.add_field(name="🎁 Artifacts Earned!", value="\n".join(artifacts_earned), inline=False)
        ...
        if alive_members == 0:
            ...
            embed = discord.Embed(
                ...
                "*Grow stronger and return to challenge the boss again!*",
                color=discord.Color.dark_red(),
            )
            await ctx.send(embed=embed)
            active_team_boss_battle = None
        ...
        await ctx.send(
            f"🏳️ **{team['name']}** has fled from the {boss_template['emoji']} **{boss_template['name']}**!\n**Penalty:** -25 Team Score"
        )
        active_team_boss_battle = None
...
async def execute_boss_turn(ctx, team, team_members, boss_template):
    ...
    if special_triggered:
        ...
        if "aoe_multiplier" in boss_template["special"]:
            ...
            action_text = (
                f"💥 {boss_template['emoji']} **{special_name}!** AoE attack hits the entire team!\n"
                + ", ".join(hit_members)
            )
            ...
            target = await bot.fetch_user(int(target_id))
            action_text = (
                f"😵 {boss_template['emoji']} **{special_name}!** {target.display_name} is stunned for 1 turn!"
            )
        else:
        ...
        if target_data["current_hp"] <= 0:
            ...
            target_data["status"] = "ko"
            action_text = (
                f"⚔️ {boss_template['emoji']} attacks {target.display_name} for **{damage} damage**! 💀 **KO!**"
            )
        else:
...
async def team_create(ctx, *, team_name: str):
    ...
    teams_data["teams"][team_id] = {
        ...
        "base_tier": "solo",
        "base_color": "white",
        "base_modules": {"training_area": 0, "armory": 0, "garden": 0},
        "decorations": [],
        "gym_equipment": [],
        "gym_level": 0,
        "arena_level": 0,
        "duel_stats": {"wins": 0, "losses": 0, "ties": 0},
        "team_chi": 0,
        "team_score": 0,
        "allies": [],
        "enemies": [],
    ...
    if member:
        ...
        try:
            ...
            invite_embed.add_field(
                name="How to Respond",
                value="`P!team invite accept` - Join the team\n`P!team invite deny` - Decline invitation",
                inline=False,
            )
...
async def team_ally(ctx, action: str = "", target: discord.Member = None):
    ...
    if action == "add":
        ...
        await ctx.send(
            f"🤝 **{team['name']}** and **{target_team['name']}** are now allies!\n"
            "✅ Members can train together\n"
            "❌ Members cannot duel each other"
        )
...
async def team_enemy(ctx, action: str = "", target: discord.Member = None):
    ...
    if action == "add":
        ...
        await ctx.send(
            f"⚔️ **{team['name']}** and **{target_team['name']}** are now enemies!\n"
            "✅ Members can always duel each other\n"
            "❌ Members cannot train together"
        )
...
async def team_upgrade(ctx, facility: str, module: str = None):
    ...
    if facility == "base":
        if module is None:
            await ctx.send(
                "❌ Please specify a module: `training_area`, `armory`, or `garden`\n"
        ...
        await ctx.send(
            f"✅ **{module_name}** upgraded to Level {team['base_modules'][module]}!\n"
    ...
    # Gym upgrades
        ...
        await ctx.send(
            f"✅ **Gym** upgraded to Level {team['gym_level']}!\n"
            f"Cost: {cost} duel wins\nRemaining wins: {team['duel_stats']['wins']}"
        )
    ...
    # Arena upgrades
        ...
        await ctx.send(
            f"✅ **Arena** upgraded to Level {team['arena_level']}!\n"
            f"Cost: {cost} duel wins\nRemaining wins: {team['duel_stats']['wins']}"
        )
...
async def team_base(ctx, action: str = "", *, args: str = ""):
    ...
    if not action or action == "view":
        ...
        embed = discord.Embed(
            ...
            color=discord.Color.from_str(
                f"#{team.get('base_color', 'white').lower().replace('white', 'ffffff').replace('red', 'ff0000').replace('blue', '0000ff').replace('green', '00ff00').replace('yellow', 'ffff00').replace('purple', '800080').replace('orange', 'ffa500').replace('pink', 'ffc0cb').replace('black', '000000')}"
                if team.get("base_color", "white")
                in ["white", "red", "blue", "green", "yellow", "purple", "orange", "pink", "black"]
                else "#"
            ),
        )
        ...
        embed.set_footer(
            text="Use P!team base paint <color> to change color • P!team base decorate <item> to add items"
        )
        await ctx.send(embed=embed)
        return
    ...
    if action == "train":
        ...
        await ctx.send(
            f"{config['emoji']} {ctx.author.mention} trained with the **{equipment}**!\n"
    ...
    if action == "relax":
        ...
        location_config = {
            ...
            "Bed": {"cooldown": 43200, "emoji": "🛏️", "message": "You had a good rest!"},  # 12 hours
            "Couch": {
                "cooldown": 21600,  # 6 hours
                "emoji": "🛋️",
        ...
        if location == "TV":
            ...
            await ctx.send(
                f"{config['emoji']} {ctx.author.mention} rested in bed!\n"
                f"*{config['message']}*\n"
            ...
            save_data()
            await ctx.send(
                f"{config['emoji']} {ctx.author.mention} relaxed on the couch!\n"
                f"*{config['message']}*\n"
                "**+1 chi**"
            )
        return
    ...
    await ctx.send(
        "❌ Invalid action! Use `P!team base view`, `P!team base paint <color>`, `P!team base decorate <item>`, `P!team base gym <equipment>`, `P!team base train <equipment>`, or `P!team base relax <location>`"
    )
...
async def team_event(ctx, *, event_name: str = ""):
    ...
    active_team_event = {"name": event_name, "registered_teams": []}
...
async def team_register(ctx, *, event_name: str = ""):
    ...
    if not active_team_event:
        await ctx.send(
            "❌ There's no active team event right now! Wait for an admin to start one with `P!team event <name>`"
        )
        return
    ...
    if event_name.lower() != active_team_event["name"].lower():
        await ctx.send(
            f"❌ Event name doesn't match! The active event is: **{active_team_event['name']}**\n"
    ...
    await log_event(
        ...
        f"**Team:** {team['name']}\n**Event:** {active_team_event['name']}\n**Leader:** {ctx.author.mention} ({ctx.author.name})\n**Total Registered:** {len(active_team_event['registered_teams'])}",
        ctx.guild,
        discord.Color.purple(),
    )
...
async def pet_store(ctx):
    ...
    for pet_key, pet in sorted_pets:
        ...
        embed.add_field(name=f"{pet['emoji']} {pet['name']} (Tier {pet['tier']})", value=value_text, inline=False)
...
async def pet_buy(ctx, *, pet_name: str = ""):
    ...
    for owned_pet in chi_data[user_id]["pets"]:
        if owned_pet["type"] == found_pet_key:
            await ctx.send(
                f"❌ You already own a **{found_pet['name']}**! Each user can only own one of each pet type."
            )
            return
    ...
    if current_chi < found_pet["price"]:
        await ctx.send(
            f"❌ You don't have enough chi! You need **{found_pet['price']:,} chi** but only have **{current_chi:,} chi**."
        )
        return
...
async def pet_shop(ctx, action: str = "", *, item_name: str = ""):
    ...
    if not action:
        # Display shop
        embed = discord.Embed(
            title="🍱 Pet Food Shop", description="Buy food to restore your pet's health!", color=discord.Color.orange()
        )
    ...
    if action.lower() == "buy":
        ...
        if current_chi < food_data["price"]:
            await ctx.send(
                f"❌ You don't have enough chi! You need **{food_data['price']:,} chi** but only have **{current_chi:,} chi**."
            )
            return
        ...
        embed.add_field(
            name="📦 You have", value=f"{chi_data[user_id]['pet_food'][food_key]}x {food_data['name']}", inline=True
        )
...
async def pet_info(ctx):
    ...
    embed = discord.Embed(
        title=f"{pet['emoji']} {pet_name}'s Info", description=pet_data["description"], color=discord.Color.blue()
    )
...
async def pet_attacks(ctx):
    ...
    for pet_key, pet_data in sorted_pets:
        ...
        embed.add_field(
            name=f"{pet_data['emoji']} {pet_data['name']} (Tier {pet_data['tier']})", value=attacks_text, inline=False
        )
...
async def pet_list(ctx):
    ...
    embed = discord.Embed(
        title="🐾 Your Pet Collection", description=f"You own **{len(pets)}** pet(s):", color=discord.Color.purple()
    )
    ...
    for pet in pets:
        ...
        embed.add_field(name=f"{pet['emoji']} {pet_name} {is_active}", value=value_text, inline=False)
...
async def pet_switch(ctx, *, pet_name: str = ""):
    ...
    found_pet = None
    for pet in chi_data[user_id]["pets"]:
        if pet["name"].lower() == pet_name.lower() or (
            pet.get("nickname") and pet["nickname"].lower() == pet_name.lower()
        ):
            found_pet = pet
            break
...
async def pet_attack(ctx, attack_id: str = ""):
    ...
    try:
        ...
        await ctx.send(
            "❌ Invalid attack ID format! Use P# (e.g., P1, P5, P12)\nUse `P!pet attacks` to see all available attacks."
        )
        return
    ...
    if active_pet_type != attack_pet_type:
        ...
        await ctx.send(
            f"❌ You can only use {PET_DATA[attack_pet_type]['emoji']} **{pet_name_for_attack}** attacks!\n"
            f"Your active pet is {pet['emoji']} **{pet.get('nickname') or pet['name']}**.\n"
            "Use `P!pet attacks` to see which attacks you can use."
        )
        return
    ...
    await ctx.send(
        f"🐾 {ctx.author.mention}'s {pet['emoji']} **{pet_name}** used **{selected_attack['emoji']} {selected_attack['name']}** and dealt **{damage} damage** to {opponent.mention}!\n"
    ...
    if opponent_hp <= 0:
        ...
        try:
            ...
            if log_channel_id:
                event_channel = get_guild_channel_obj(ctx.guild, "log_channel_id")

            # Fallback: Use first available text channel
            if not event_channel:
                event_channel = next(
                    (ch for ch in ctx.guild.text_channels if ch.permissions_for(ctx.guild.me).send_messages), None
                )
...
async def pet_train(ctx, training_type: str = ""):
    ...
    if training_type == "heal":
        ...
        if chi_data[user_id].get("chi", 0) < cost:
            await ctx.send(
                f"❌ You need **{cost} chi** to train healing! You have **{chi_data[user_id].get('chi', 0)} chi**."
            )
            return
        ...
        if health_level >= 1:
            await ctx.send(
                f"❌ **{pet_name}** has already received the Peace Orb health upgrade! This can only be done once."
            )
            return
        ...
        if not peace_orb_owned:
            await ctx.send("❌ You need a **Peace Orb** to upgrade health! Buy it from `P!rshop` (R8).")
            return
        ...
        await ctx.send(
            f"✨ **{pet_name}** consumed the Peace Orb!\n"
        ...
        if not user_artifacts:
            await ctx.send("❌ You need an artifact to train attack! Find artifacts with `P!find artifact`.")
            return
        ...
        await ctx.send(
            f"✨ **{pet_name}** consumed {consumed_artifact['emoji']} **{consumed_artifact['name']}**!\n"
        ...
        if garden_level < 5:
            await ctx.send(
                f"❌ Your garden must be max level (5) for special training! Current level: **{garden_level}**/5"
            )
            return
        ...
        if chi_data[user_id].get("chi", 0) < cost:
            await ctx.send(
                f"❌ You need **{cost} chi** for special training! You have **{chi_data[user_id].get('chi', 0)} chi**."
            )
            return
        ...
        await ctx.send(
            f"✨ **{pet_name}** completed special training!\n"
...
async def pay(ctx, target: discord.Member = None, amount: str = "", currency: str = "chi"):
    ...
    if not target or not amount:
        await ctx.send(
            "❌ Usage: `P!pay @user <amount> [chi/rebirth]`\n"
            "Examples:\n"
            "`P!pay @user 100` - Pay 100 chi\n"
            "`P!pay @user 100 chi` - Pay 100 chi\n"
            "`P!pay @user 1 rebirth` - Pay 1 rebirth"
    ...
    if currency_type == "rebirth":
        ...
        if sender_rebirths < pay_amount:
            await ctx.send(
                f"❌ You don't have enough rebirths! You have **{sender_rebirths}** rebirth(s) but tried to pay **{pay_amount}**."
            )
            return
        ...
        embed.add_field(
            name=f"{ctx.author.display_name}'s Rebirths", value=str(chi_data[sender_id]["rebirths"]), inline=True
        )
        embed.add_field(
            name=f"{target.display_name}'s Rebirths", value=str(chi_data[receiver_id]["rebirths"]), inline=True
        )
        await ctx.send(embed=embed)
        ...
        if sender_chi < pay_amount:
            await ctx.send(
                f"❌ You don't have enough chi! You have **{sender_chi}** chi but tried to pay **{pay_amount}**."
            )
            return
...
async def search_shop(ctx, shop_type: str = "", category: str = "", price: str = ""):
                ...
                results.append(
                    {
                        ...
                        "emoji": (
                            "⚔️"
                            if "sword" in item["name"].lower() or "katana" in item["name"].lower()
                            else (
                                "🍵"
                                if "tea" in item["name"].lower()
                                else "🧥" if "coat" in item["name"].lower() or "hat" in item["name"].lower() else "📦"
                            )
                        ),
                ...
                results.append(
                    {
                        ...
                        "emoji": (
                            "📈"
                            if "level" in item["name"].lower()
                            else (
                                "🎨"
                                if "role" in item["name"].lower() or "decoration" in item["name"].lower()
                                else "💎" if "nitro" in item["name"].lower() else "📦"
                            )
                        ),
                        "description": f"Type: {item_type}",
                    }
    ...
    if not results:
        await ctx.send(
            f"❌ No items found in **{shop_name}** for category/price: `{category}`\n\n"
            "Try searching by:\n"
            "• Category: `seeds`, `tools` (Garden Shop)\n"
            "• Type: `default` (Chi/Rebirth Shop)\n"
            "• Price: Any number"
        )
        return
...
async def garden(ctx):
    ...
    if user_id not in gardens_data["gardens"]:
        ...
        if chi_data[user_id].get("chi", 0) < 50:
            await ctx.send(
                "🌸 **You need 50 chi to create your garden!**\n\n"
        ...
        await ctx.send(
            f"{tier_info['emoji']} **Garden Created!**\n"
            f"You've created a **{tier_info['name']}** for 50 chi!\n\n"
            "Use `P!garden shop` to buy seeds and tools!\n"
            "Plant seeds with `P!garden plant <seed_name>`"
        )
        return
    ...
    if not isinstance(chi_data[user_id]["garden_inventory"], dict):
        print(
            f"⚠️ CRITICAL ERROR: garden_inventory corrupted for user {user_id}! Type: {type(chi_data[user_id]['garden_inventory']).__name__}"
        )
        chi_data[user_id]["garden_inventory"] = {}
        save_data()
    ...
    if garden_data["plants"]:
        ...
        for plant in garden_data["plants"]:
            ...
            grow_minutes = calculate_grow_time(
                plant_info["grow_time_minutes"], has_fertilizer, garden_tier, user_id, plant_name, garden_data
            )
        ...
        max_capacity = get_garden_max_capacity(garden_data.get("tier", "rare"))
        embed.add_field(
            name=f"🌿 Your Plants (0/{max_capacity})",
            value="No plants yet! Use `P!garden shop` to buy seeds.",
            inline=False,
        )
    ...
    if garden_inv:
        ...
        if inv_list:
            embed.add_field(
                name="🎒 Garden Inventory", value="\n".join(inv_list) + "\n\n*View all in `P!inv`*", inline=False
            )

    # Show garden event status
    if is_garden_event_active():
        embed.add_field(
            name="✨ Spiritual Journey Boost Active!", value="All plants are instantly ready to harvest!", inline=False
        )
...
async def garden_shop(ctx, action: str = "", item_name: str = "", quantity: int = 1):
    ...
    if action.lower() == "buy" and item_name:
        ...
        if user_chi < total_cost:
            await ctx.send(
                f"❌ You need **{total_cost:,} chi** to buy **{quantity}x {item_name}**! You have **{user_chi:,} chi**."
            )
            return
        ...
        if item_category == "seeds":
            ...
            item_info = GARDEN_SHOP_ITEMS[item_category][item_name]
            chi_data[user_id]["garden_inventory"][item_name] = (
                chi_data[user_id]["garden_inventory"].get(item_name, 0) + quantity
            )
            purchase_message = (
                f"✅ Purchased **{quantity}x {item_info['emoji']} {item_name}** for **{total_cost:,} chi**!"
            )
        ...
        # Show shop
        embed = discord.Embed(
            title="🌸 Garden Shop", description="Purchase seeds and tools for your garden!", color=discord.Color.green()
        )
        ...
        for idx, (tool_name, tool_info) in enumerate(GARDEN_SHOP_ITEMS["tools"].items(), start=1):
            tools_list.append(
                f"G{idx}: {tool_info['emoji']} **{tool_name}** - {tool_info['cost']} chi\n*{tool_info['description']}*"
            )
        ...
        for idx, (seed_name, seed_info) in enumerate(GARDEN_SHOP_ITEMS["seeds"].items(), start=tool_count + 1):
            ...
            seeds_list.append(
                f"G{idx}: {seed_info['emoji']} **{seed_name}** - {seed_info['cost']} chi{lock_text}\n"
        ...
        embed.set_footer(
            text="Use P!garden shop buy <item_name or G#> [quantity] to purchase • Example: P!garden shop buy G1 5"
        )
        await ctx.send(embed=embed)
...
async def garden_plant(ctx, action: str = "", *, seed_identifier: str = ""):
    ...
    if action.lower() == "all":
        await ctx.send(
            "❌ The `plant all` command has been removed.\n"
    ...
    if current_plant_count + quantity > max_capacity:
        ...
        await ctx.send(
            f"❌ Your garden can only hold **{max_capacity}** total plants!\n"
            f"Current: **{current_plant_count}** plants\n"
            f"Trying to plant: **{quantity}** seeds\n"
            f"Available space: **{remaining_capacity}** seeds\n\n"
            "💡 Harvest some plants first with `P!garden harvest all` to make space, or upgrade your garden tier!"
        )
        return
    ...
    if current_seeds < quantity:
        await ctx.send(
            f"❌ You don't have enough **{plant_name}** seeds!\n"
            f"You have **{current_seeds}**, need **{quantity}**\n"
            "Buy more from `P!garden shop`"
        )
        return
    ...
    seed_info = GARDEN_SHOP_ITEMS["seeds"][plant_name]
    for _ in range(quantity):
        garden_data["plants"].append({"name": plant_name, "planted_at": datetime.utcnow().timestamp()})
    save_gardens()
    ...
    garden_tier = garden_data.get("tier", "rare")
    grow_minutes = calculate_grow_time(
        seed_info["grow_time_minutes"], has_fertilizer, garden_tier, user_id, plant_name, garden_data
    )
    ...
    bonus_text = (
        (tier_text + fertilizer_text + pet_text + water_text)
        if (tier_text or fertilizer_text or pet_text or water_text)
        else ""
    )
    ...
    await ctx.send(
        f"🌱 Planted **{quantity_text}{seed_info['emoji']} {plant_name}**!\n"
        f"Growth time: **{grow_minutes:.1f} minutes**{bonus_text}\n"
        f"Harvest reward: **+{seed_info['harvest_chi']} chi** each\n"
        "💡 Harvest to get seeds back (10% chance of bonus seed!)\n"
...
async def garden_water(ctx, *, plant_name: str = ""):
    ...
    if not has_bucket and not has_golden:
        await ctx.send(
            "❌ You need a **Watering Bucket** or **Golden Watering Can** to water plants! Buy one from `P!garden shop`."
        )
        return
    ...
    await ctx.send(
        f"💧 Watered **{seed_info['emoji']} {plant_name}** with your {can_type}!\n"
        "✨ This plant will grow **5% faster** for the next 24 hours! 🌿"
    )
...
async def garden_fertilize(ctx):
    ...
    embed.add_field(name="Fertilizer Bag Status", value=f"🧪 **{uses_remaining}/10 uses remaining**", inline=False)
...
async def garden_harvest(ctx, *, plant_name: str = ""):
    ...
    if random.random() < (0.60 + pet_rare_bonus):
        ...
        common_herbs = ["moonpetal", "winterlea", "sunbloom"]
    ...
    await ctx.send(
        f"✨ Harvested **{seed_info['emoji']} {plant_name}**!\n"
...
async def garden_harvest_all(ctx, *, plant_name: str = ""):
    ...
    if not ready_plants:
        ...
        minutes_left = int(time_left.total_seconds() / 60)
        await ctx.send(
            f"🌱 None of your **{plant_name}** plants are ready yet!\n" f"Next harvest in **{minutes_left} minutes**."
        )
        return
    ...
    if ingredient_drops:
        ingredient_text = ", ".join(
            [
                f"🧪 {qty}x {ing_emoji} {name}"
                for name, (ing_emoji, qty) in {
...
async def garden_seed_info(ctx, *, seed_name: str = ""):
    ...
    embed = discord.Embed(
        title=f"{seed_info['emoji']} {seed_name}", description=seed_info["description"], color=discord.Color.green()
    )
...
async def garden_about(ctx, user: discord.Member = None):
    ...
    for plant in garden_data["plants"]:
        ...
        grow_minutes = calculate_grow_time(
            plant_info["grow_time_minutes"], has_fertilizer, garden_tier, user_id, plant_name_var, garden_data
        )
        ready_time = planted_time + timedelta(minutes=grow_minutes)
...
async def garden_upgrade(ctx):
    ...
    if current_index >= len(tier_progression) - 1:
        await ctx.send(
            f"✨ Your **{current_tier_info['emoji']} {current_tier_info['name']}** is already at the maximum tier!"
        )
        return
    ...
    if not can_afford:
        await ctx.send(
            f"❌ You cannot afford the upgrade to **{next_tier_info['emoji']} {next_tier_info['name']}**!\n\n"
            f"**Cost:** {cost_display}\n"
    ...
    benefits = []
    benefits.append(
        f"📦 **Max Capacity:** {next_tier_info['max_capacity']} plants (+{next_tier_info['max_capacity'] - current_tier_info['max_capacity']})"
    )
...
async def trade(ctx, trade_type: str = "", target: discord.Member = None, *, amount_or_item: str = ""):
    ...
    if not trade_type or not target or not amount_or_item:
        await ctx.send(
            "❌ Usage: `P!trade <chi/artifact/pet> <@user> <amount/item_name>`\n"
            "Examples:\n"
    ...
    if trade_type == "chi":
        ...
        if chi_data[user_id].get("active_pet") == pet_to_trade["type"]:
            await ctx.send("❌ You can't trade your active pet! Use `P!pet switch <name>` to switch first.")
            return
...
async def petfood_shop(ctx):
    ...
    if inventory:
        ...
        if inv_items:
            embed.add_field(name="📦 Your Inventory", value="\n".join(inv_items[:10]), inline=False)  # Show first 10

    embed.add_field(name="💰 Your Chi", value=f"{chi_data[user_id].get('chi', 0):,} chi", inline=True)
    ...
    select = discord.ui.Select(
        placeholder="🛒 Choose food to purchase...", options=options[:25], custom_id="petfood_purchase"  # Discord limit
    )
    ...
    async def purchase_callback(interaction: discord.Interaction):
        ...
        conf_embed.add_field(
            name="📦 You now have",
            value=f"{chi_data[user_id]['pet_food'][selected_food]}x {food_data['name']}",
            inline=False,
        )
...
async def pshop(ctx):
    ...
    if not user_shop_id:
        ...
        embed.add_field(
            name="📝 Getting Started", value="`P!pshop create <shop name>` - Create your shop", inline=False
        )
        embed.add_field(
            name="🔍 Browse Shops",
            value=(
...
async def view_shop_display(ctx, shop_id, is_owner=False):
    ...
    if not shop["listings"]:
        embed.add_field(name="📦 Listings", value="*This shop has no items for sale*", inline=False)
    else:
...
async def pshop_create(ctx, *, shop_name: str = ""):
    ...
    for shop_id, shop_data in player_shops_data["shops"].items():
        if str(shop_data["owner_id"]) == user_id:
            await ctx.send("❌ You already own a shop! View it with `P!pshop`")
            return
...
async def pshop_sell(ctx):
    ...
    embed.add_field(
        name="📦 Your Inventory", value=f"{len([i for i in inventory.values() if i > 0])} items available", inline=True
    )
    embed.add_field(name="🏷️ Current Listings", value=f"{len(shop['listings'])} items listed", inline=True)

    view = PlayerShopInventorySelect(
        user_id, inventory, user_shop_id, chi_data, player_shops_data, chi_shop_data, save_data, save_player_shops
    )
...
async def pshop_buy(ctx, shop_id: str = "", *, item_name: str = ""):
    ...
    if not found_listing:
        await ctx.send(
            f"❌ **{item_name}** is not for sale in this shop! Use `P!pshop view {shop_id}` to see available items."
        )
        return
    ...
    # Transfer item
    chi_data[user_id]["inventory"][found_listing["item_name"]] = (
        chi_data[user_id]["inventory"].get(found_listing["item_name"], 0) + 1
    )
    ...
    embed.add_field(name="Remaining Chi", value=f"{chi_data[user_id]['chi']:,}", inline=True)
    embed.set_footer(text="Item added to your inventory! Check with P!inventory")
...
async def pshop_list(ctx):
    ...
    sorted_shops = sorted(player_shops_data["shops"].items(), key=lambda x: x[1].get("total_sales", 0), reverse=True)
    ...
    for i, (shop_id, shop) in enumerate(sorted_shops[:20], 1):  # Top 20 shops
        ...
        embed.add_field(name=f"{i}. {shop['name']} ({shop_id})", value=value_text, inline=False)
...
async def pshop_stats(ctx):
    ...
    embed = discord.Embed(
        title=f"📊 {shop['name']} Statistics", description=f"Shop ID: {user_shop_id}", color=discord.Color.blue()
    )
    ...
    # Average price
    if shop["listings"]:
        avg_price = sum(listing["price"] for listing in shop["listings"]) / len(shop["listings"])
...
async def potion_list(ctx):
    ...
    for rarity, potions in rarities.items():
        if potions:
            embed.add_field(name=rarity_colors.get(rarity, rarity.title()), value="\n".join(potions), inline=False)
...
async def potion_inventory(ctx):
    ...
    for potion_key, count in potions.items():
        ...
        if potion:
            embed.add_field(
                name=f"{potion['emoji']} {potion['name']} x{count}", value=potion["description"], inline=False
            )
...
async def potion_info(ctx, *, potion_name: str):
    ...
    embed = discord.Embed(
        title=f"{potion['emoji']} {potion['name']}", description=potion["description"], color=discord.Color.purple()
    )
...
async def brew_list(ctx):
    ...
    for tier, recipes in tiers.items():
        if recipes:
            embed.add_field(name=tier_labels.get(tier, tier.title()), value="\n".join(recipes), inline=False)
...
async def brew_ingredients(ctx):
    ...
    for source, items in sources.items():
        if items:
            embed.add_field(name=source_labels.get(source, source.title()), value="\n".join(items), inline=False)
...
async def brew_craft(ctx, *, potion_name: str):
    ...
    try:
        ...
        # Start brewing
        _ = await ctx.send(f"🧙 Brewing {potion['emoji']} **{potion['name']}**... ⏰ {recipe['brew_time']}s")
        ...
        if success:
            ...
            fail_embed = discord.Embed(
                ...
                description="The potion exploded! Ingredients and chi were lost.\n\n" "Better luck next time...",
                color=discord.Color.red(),
            )
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send(f"❌ Missing required argument: `{error.param.name}`\n" "Use `P!help` for command usage info.")
        return
    ...
    if isinstance(error, commands.CommandInvokeError):
        ...
        if isinstance(original, KeyError):
            await ctx.send(
                "⚠️ A data error occurred. Please try again or contact an admin if this persists.\n"
        ...
        await ctx.send(
            "⚠️ An error occurred while executing this command. Please contact an admin.\n" f"Error: {str(original)}"
        )
